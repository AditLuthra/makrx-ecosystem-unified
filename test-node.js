...
if (fs.existsSync(appsDir)) {
    ...
    apps.forEach(app => {
        ...
        console.log(`  ${hasNodeModules ? '✅' : '❌'} ${app}: ${hasNodeModules ? 'Ready' : 'Missing dependencies'}`);
...
if (fs.existsSync(backendsDir)) {
    const backends = fs.readdirSync(backendsDir).filter(name => 
        fs.statSync(path.join(backendsDir, name)).isDirectory()
    );
