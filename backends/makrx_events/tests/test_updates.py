...
def test_event_team_sponsor_tournament_updates():
    ...
    r2 = client.patch(f"/api/events/{event_id}", json={"title": "Renamed Event", "status": "published"})
    ...
    # Patch team name
    r4 = client.patch(f"/api/events/{event_id}/teams/{team['id']}", json={"name": "Alpha Prime"})
    assert r4.status_code == 200, r4.text
    team2 = r4.json()
    assert team2["name"] == "Alpha Prime"

    # Create sponsor
    r5 = client.post(f"/api/events/{event_id}/sponsors", json={"event_id": event_id, "name": "ACME"})
    assert r5.status_code == 201, r5.text
    sponsor = r5.json()

    # Patch sponsor
    r6 = client.patch(f"/api/events/{event_id}/sponsors/{sponsor['id']}", json={"status": "inactive", "tier": "gold"})
