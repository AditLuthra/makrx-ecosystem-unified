...
class Upload(Base):
    ...
    # Relationships
    quotes = relationship(
        "Quote", back_populates="upload", cascade="all, delete-orphan"
    )
...
class Quote(Base):
    ...
    settings = Column(
        JSONB, default={}
    )  # printer-specific settings, speeds, temps, etc.
...
class ServiceOrderLog(Base):
    ...
    )  # status_change, note_added, file_uploaded
    ...
    # Additional data (use column name 'metadata' but avoid reserved attribute)
    ...
    __table_args__ = (
        Index(
            "ix_service_order_logs_order_created",
            "service_order_id",
            "created_at",
        ),
    )
