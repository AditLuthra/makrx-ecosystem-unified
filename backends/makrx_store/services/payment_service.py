...
class PaymentService:
    ...
    async def _create_manual_upi_payment(
        self, payment_id: str, request: PaymentRequest
    ) -> PaymentResponse:
    ...
    async def _create_cod_payment(
        ...
        if request.amount < 100:
            raise HTTPException(
                status_code=400, detail="COD minimum order value is ₹100"
            )

        if request.amount > 5000:
            raise HTTPException(
                status_code=400, detail="COD maximum order value is ₹5000"
            )
        ...
        if not self._is_cod_available(pincode):
            raise HTTPException(
                status_code=400, detail="COD not available for this location"
            )
    ...
    async def verify_payment(
        self, payment_id: str, gateway_data: Dict[str, Any]
    ) -> bool:
    ...
    async def _create_razorpay_refund(self, request: RefundRequest) -> RefundResponse:
        ...
        try:
            ...
            refund_data = {
                "amount": (
                    int(request.amount * 100) if request.amount else None
                ),  # Convert to paise
                "notes": {
            ...
            raise HTTPException(
                status_code=400, detail=f"Razorpay refund failed: {str(e)}"
            )
    ...
    def calculate_platform_fee(
        self, amount: float, payment_method: PaymentMethod
    ) -> float:
    ...
    def get_supported_methods(
        self, amount: float, country: str = "IN"
    ) -> List[PaymentMethod]:
