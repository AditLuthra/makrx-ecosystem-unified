...
async def update_inventory(
    ...
    if update.update_type == "consumed":
        ...
        await db.execute(
            ...
            .values(current_stock=new_current, reserved_stock=new_reserved)
        )
    ...
    for item in low_stock_items:
        ...
        catalog_result = await db.execute(
            select(MaterialCatalog).where(
                MaterialCatalog.material_id == item.material_type
            )
        )
...
async def get_usage_analytics(
    ...
    total_revenue = sum(
        job.customer_price * 0.7 for job in completed_jobs
    )  # Provider gets ~70%
        ...
        inventory_summary[item.material_type] = {
            ...
            "days_remaining": (
                (
                    item.current_stock
                    / (
                        material_usage.get(item.material_type, {}).get(
                            "estimated_material_used", 0.1
                        )
                        / days
                    )
                )
                if material_usage.get(item.material_type, {}).get(
                    "estimated_material_used", 0
                )
                > 0
                else 999
    ...
    return {
        ...
        "recommendations": generate_inventory_recommendations(
            material_usage, inventory_summary
        ),
    }
...
async def check_inventory_alerts(
    provider_id: str, inventory_item: ProviderInventory, db: AsyncSession
):
...
async def log_reorder_activity(
    provider_id: str, material_id: str, quantity: float, reorder_url: str
):
    """Log reorder activity for analytics"""
    # This would log to an activity/audit table
    print(
        f"Reorder logged: Provider {provider_id} reordered {quantity} units of {material_id}"
    )
