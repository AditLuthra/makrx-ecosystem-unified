...
from ..models.commerce import Product, Cart, CartItem
from ..models.subscriptions import BOMIntegration
...
async def import_bom_from_makrcave(
    ...
    try:
        ...
        bom_data = await fetch_bom_from_makrcave(
            request.makrcave_project_id, request.makrcave_user_id
        )
        ...
        for bom_item in bom_data.items:
            ...
            if result.best_match:
                mapped_count += 1
                estimated_total += (
                    result.best_match.store_product_price * bom_item.quantity
                )
            else:
                unmapped_count += 1
    ...
    try:
        ...
        for mapping in request.mappings:
            ...
            pres = await db.execute(
                select(Product).where(Product.id == mapping.store_product_id)
            )
...
async def list_user_imports(
    ...
    try:
        stmt = (
            select(BOMIntegration)
            .where(BOMIntegration.makrcave_user_id == user_id)
            .order_by(BOMIntegration.created_at.desc())
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"MakrCave API error: {response.status_code} - {response.text}"
                )
                return None
...
async def map_bom_item_to_products(
    db: AsyncSession, bom_item: BOMItem
) -> BOMMappingResult:
    ...
    if bom_item.part_number:
        ...
        exact_matches = (await db.execute(exact_stmt)).scalars().all()
    ...
    if bom_item.manufacturer and not matches:
        nb_stmt = select(Product).where(
            and_(
                Product.is_active == True,
                Product.brand.ilike(f"%{bom_item.manufacturer}%"),
                Product.name.ilike(f"%{bom_item.name[:20]}%"),
            )
        )
        name_brand_matches = (await db.execute(nb_stmt)).scalars().all()
            ...
            if confidence > 0.7:
                matches.append(
                    ProductMatch(
                        ...
                        * 0.8,  # Reduce confidence for partial match
                        match_type="partial",
                        store_product_id=product.id,
                        store_product_name=product.name,
                        store_product_brand=product.brand,
    ...
    if not matches:
        ...
        fuzzy_matches = (await db.execute(fuzzy_stmt)).scalars().all()
    ...
    if matches:
        ...
        in_stock_matches = [
            m for m in matches if m.quantity_available >= bom_item.quantity
        ]
...
async def get_or_create_user_cart(db: AsyncSession, user_id: str) -> Cart:
    """Get existing cart or create new one for user"""
    cart = (
        (await db.execute(select(Cart).where(Cart.user_id == user_id)))
        .scalars()
        .first()
    )
...
async def create_quick_reorder_from_bom(
    ...
    try:
        ...
        logger.info(
            f"Creating quick reorder for user {user_id} from BOM import {integration_id}"
        )
