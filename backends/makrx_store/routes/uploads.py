...
class FileProcessor:
    ...
    def validate_file(filename: str, content_type: str, file_size: int) -> List[str]:
        ...
        if file_ext not in ALLOWED_EXTENSIONS:
            errors.append(
                f"Unsupported file type: {file_ext}. Supported: {', '.join(ALLOWED_EXTENSIONS)}"
            )
    ...
    def analyze_mesh_file(file_path: str) -> Dict[str, any]:
        ...
        try:
            ...
            has_unreferenced_vertices = len(mesh.vertices) != len(
    ...
    def _estimate_print_time(
        volume_mm3: float, surface_area_mm2: float, complexity: float
    ) -> float:
        ...
        time_minutes = (
            (volume_mm3 / base_speed_mm3_per_minute)
            * surface_factor
            * complexity_factor
        )
...
async def create_upload_url(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create upload URL: {str(e)}"
        )
    ...
    try:
        ...
        try:
            ...
            try:
                    ...
                    "material_analysis": {
                        ...
                        "recommended_materials": [
                            ...
                            },
                            {
                                "material": "ABS",
                                "score": 70,
                                "reasons": ["Good for mechanical parts"],
                            },
                        ],
                    },
                }
                ...
                logger.warning(
                    f"Advanced analysis failed, using basic analysis: {analysis_error}"
                )
                analysis_result = {
                    "volume_mm3": 15000.0,
                    "surface_area_mm2": 8500.0,
...
async def get_upload_status(
    ...
    try:
        ...
        result = {
            ...
            "expires_at": (
                upload.expires_at.isoformat() if upload.expires_at else None
            ),
        }
