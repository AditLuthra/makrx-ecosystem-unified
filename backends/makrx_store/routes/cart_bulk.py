...
class BulkCartItem(BaseModel):
    ...
    project_reference: Optional[str] = Field(
        None, description="Reference to source project"
    )
    unit_price_override: Optional[float] = Field(
        None, description="Override unit price"
    )
...
class BulkCartRequest(BaseModel):
    ...
    notes: Optional[str] = Field(
        None, description="General notes for the bulk operation"
    )
...
async def bulk_add_to_cart(
    ...
    try:
        ...
        results = await process_bulk_cart_additions(
            cart, validation_result, bulk_request, db
        )
...
async def validate_bulk_cart_items(
    items: List[BulkCartItem], db: AsyncSession
...
async def get_or_create_user_cart(user_email: str, db: AsyncSession) -> Cart:
...
async def process_bulk_cart_additions(
    ...
    # Refresh cart to ensure items relationship is loaded
    await db.refresh(cart)
    existing_items = {item.product_sku: item for item in (cart.items or [])}
...
async def recalculate_cart_totals(cart: Cart, db: AsyncSession):
...
async def notify_bulk_cart_addition(
    user_email: str, project_name: str, added_items: int, updated_items: int
):
...
async def validate_bulk_cart_items_endpoint(
    items: List[BulkCartItem], db: AsyncSession = Depends(get_db)
):
    """
