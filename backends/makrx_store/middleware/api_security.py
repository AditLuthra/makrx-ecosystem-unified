...
class APISecurityConfig:
    ...
    SECURITY_HEADERS = {
        ...
        # CSP: production is strict, dev allows inline for hot reload
...
class HTTPSRedirectMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        headers = APISecurityConfig.SECURITY_HEADERS.copy()
        if settings.ENVIRONMENT == "production":
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_rate_limit(self, key: str, limit: int, window: int) -> bool:
        ...
        try:
            ...
            # Use sliding window with Redis sorted sets (async)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        rate_config = APISecurityConfig.RATE_LIMITS.get(
            endpoint_type, APISecurityConfig.RATE_LIMITS["api"]
        )

        # In production, if Redis is unavailable, fail closed on critical endpoints
        if not self.redis_client and settings.ENVIRONMENT == "production":
            critical = {"upload", "quote", "checkout", "login", "webhook"}
            if endpoint_type in critical:
                logger.error(
                    "Rate limiting backend unavailable in production; blocking critical endpoint request"
                )
                return JSONResponse(
        ...
        if user_id:
            ...
            if not user_allowed:
                logger.warning(
                    f"Rate limit exceeded for user {user_id} on {endpoint_type}"
                )
                return JSONResponse(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    content={
        ...
        if request.method in ["GET", "HEAD", "OPTIONS"]:
            ...
            if "csrf_token" not in request.cookies:
                ...
                response.set_cookie(
                    "csrf_token",
                    csrf_token,
                    httponly=False,  # Double-submit: must be JS-readable
...
class FileUploadSecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def _verify_file_type(
        ...
        if ext in APISecurityConfig.ALLOWED_STL_TYPES:
        ...
        return (
            False,
            f"Invalid file type. Extension: {ext}, Detected: {detected_type}",
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if content_length > APISecurityConfig.MAX_UPLOAD_SIZE:
            return JSONResponse(
                ...
                content={
                    "detail": f"File too large. Max size: {APISecurityConfig.MAX_UPLOAD_SIZE} bytes"
                },
            )
...
class APIGatewayMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not self._is_path_allowed(request.url.path):
            logger.warning(
                f"Blocked path access: {request.url.path} from {request.client.host}"
            )
            return JSONResponse(
...
def setup_api_security(app: FastAPI):
    ...
    """
    # Adjust CSP per environment: stricter in production
    if settings.ENVIRONMENT == "production":
        APISecurityConfig.SECURITY_HEADERS["Content-Security-Policy"] = (
            "default-src 'self'; "
    ...
    if redis_url:
        ...
        if settings.ENVIRONMENT == "production":
            logger.error(
                "REDIS_URL is required in production for rate limiting and CSRF; critical endpoints will fail closed"
            )
        else:
            logger.warning(
                "REDIS_URL not set; rate limiting will fail open in development"
            )
