...
class PricingEngine:
    ...
    def calculate_quote(
        ...
        try:
            ...
            material_rate = self.material_rates.get(
                material.lower(), self.material_rates["pla"]
            )
            material_density = Decimal(
                str(self.material_densities.get(material.lower(), 1.24))
            )
            ...
            estimated_weight_g = float(
                volume_cm3 * material_density * infill_factor * quantity
            )
            ...
            quality_multiplier = Decimal(
                str(self.quality_multipliers.get(quality, 1.0))
            )
            ...
            # Calculate subtotal
            subtotal = (
                material_cost + machine_cost + labor_cost + support_cost
            ) * rush_multiplier
            ...
            breakdown = {
                ...
                "rush_surcharge": float(
                    (subtotal * (rush_multiplier - Decimal("1.0")))
                ),
    ...
    def _calculate_labor_cost(
        self, quantity: int, supports: bool, quality: str
    ) -> Decimal:
...
def validate_print_parameters(
    material: str, quality: str, layer_height: float, infill_percentage: int
