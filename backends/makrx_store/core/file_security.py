...
class SecureFileValidator:
    ...
    async def validate_file(
        ...
        try:
            ...
            is_valid, file_type, error = self._cross_validate(
                ext, detected_mime, detected_type, file_size
            )
    ...
    def _cross_validate(
        ...
        ext: str,
        mime: str,
        magic_type: Optional[FileType],
        file_size: int,
    ) -> Tuple[bool, Optional[FileType], str]:
        ...
        if ext in [".stl", ".3mf"]:
            if file_size > FileSecurityConfig.MAX_STL_SIZE:
                return (
                    False,
                    None,
                    f"STL/3MF file too large: {file_size} bytes",
                )
        ...
        if magic_type and magic_type != expected_type:
            ...
            if not (ext == ".3mf" and magic_type == FileType.THREE_MF):
                return (
                    False,
                    None,
                    f"File content doesn't match extension. Expected: {expected_type}, Got: {magic_type}",
                )
...
class SecureStorageManager:
    ...
    async def generate_upload_url(
        self, filename: str, file_type: FileType, user_id: str
    ) -> Dict[str, Any]:
        """
    ...
    async def generate_download_url(
        self, file_key: str, user_id: str, single_use: bool = False
    ) -> Dict[str, Any]:
        """
...
class WatermarkService:
    ...
    async def watermark_image(
        image_content: bytes, watermark_text: str = "MakrX"
    ) -> bytes:
