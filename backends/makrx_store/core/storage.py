        ...
        try:
            ...
            client = boto3.client(
                ...
                endpoint_url=(
                    ...
                    if not settings.S3_ENDPOINT.startswith("https://s3.")
                    else None
                ),
            ...
            # Optionally test connection if credentials provided
            try:
                ...
                logger.warning(
                    "S3 credentials not configured; skipping bucket head check"
                )
    ...
    async def upload_file_directly(
        ...
        try:
            ...
            self.client.upload_fileobj(
                file_obj, self.bucket, file_key, ExtraArgs=extra_args
            )
...
def get_storage() -> Optional[StorageClient]:
    global _storage
    if _storage is None:
        try:
            _storage = StorageClient()
        except Exception as e:
            logger.warning(f"Deferring storage init: {e}")
            _storage = None
    return _storage


async def upload_file_to_storage(
    file_obj: BinaryIO, filename: str, prefix: str = "uploads"
) -> str:
    storage = get_storage()
    if not storage:
...
async def generate_presigned_url(file_key: str, expires_in: int = 3600) -> str:
    ...
    if not storage:
        raise Exception("Storage not configured")
    return await storage.generate_presigned_download_url(file_key, expires_in)


def generate_file_key(
    user_id: Optional[str], file_extension: str, prefix: str = "uploads"
) -> str:
