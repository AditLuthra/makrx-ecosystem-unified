...
class InvoiceGenerator:
    ...
    def __init__(self):
        ...
        self.company_info = {
            ...
            "address": os.getenv(
                "COMPANY_ADDRESS", "123 Maker Street\nTech City, TC 12345"
            ),
    ...
    def _build_header(self, invoice_data: Dict[str, Any]) -> List:
        ...
        invoice_info = [
            ...
            Paragraph(
                f"<b>Invoice #: {invoice_data.get('invoice_number', 'N/A')}</b>",
                self.normal_style,
            ),
            Paragraph(
                f"Date: {self._format_date(invoice_data.get('issue_date'))}",
                self.normal_style,
            ),
            Paragraph(
    ...
    def _build_line_items(self, invoice_data: Dict[str, Any]) -> List:
        ...
        if not line_items:
            description = invoice_data.get(
                "description", invoice_data.get("title", "Service")
            )
            line_items_data.append(
                [
                    description,
    ...
    def _build_totals(self, invoice_data: Dict[str, Any]) -> List:
        ...
        totals_data.append(
            [
                "<b>Total:</b>",
                f"<b>{self._format_currency(total_amount, currency)}</b>",
            ]
        )
    ...
    def _build_payment_info(self, invoice_data: Dict[str, Any]) -> List:
        ...
        if invoice_data.get("paid_date"):
            ...
            due_date = datetime.fromisoformat(
                str(invoice_data["due_date"]).replace("Z", "+00:00")
            )
...
def generate_invoice_pdf(
    invoice_data: Dict[str, Any], output_path: str = None
) -> bytes:
