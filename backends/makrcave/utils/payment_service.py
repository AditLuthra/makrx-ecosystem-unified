...
class PaymentService:
    ...
    def verify_payment(
        ...
        if gateway.lower() == "razorpay":
            ...
            if is_valid:
                payment_details = self.razorpay.get_payment(
                    payment_data["razorpay_payment_id"]
                )
                return True, {
            ...
            try:
                intent = self.stripe.confirm_payment_intent(
...
def calculate_service_charge(
    amount: float, service_type: str, gateway: str = "razorpay"
) -> Tuple[float, float, float]:
    """Calculate service charges and taxes"""

    # Gateway charges (approximate)
    gateway_charges = {
        "razorpay": {"percentage": 2.0, "fixed": 0},  # 2% + GST
        "stripe": {"percentage": 2.9, "fixed": 0.30},  # 2.9% + 30 cents
    }
