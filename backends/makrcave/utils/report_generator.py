...
class ReportGenerator:
    ...
    def generate_usage_report_csv(
        ...
        summary_data.append(
            [
                "Most Common Event",
                df["Event Type"].mode().iloc[0] if not df.empty else "N/A",
            ]
        )
        summary_data.append(
            [
                "Total Duration (hours)",
                round(df["Duration (minutes)"].sum() / 60, 2),
            ]
        )
    ...
    def generate_inventory_report_xlsx(
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            ...
            top_consumers = sorted(
                ...
                key=lambda x: x[1]["consumed"],
                reverse=True,
            )[:20]
            top_consumer_data = [
                {
                    "Item ID": item_id,
                    "Total Consumed": data["consumed"],
                    "Total Cost": f"${data['cost']:.2f}",
                }
                for item_id, data in top_consumers
            ]
            pd.DataFrame(top_consumer_data).to_excel(
                writer, sheet_name="Top Consumers", index=False
            )

        return filepath

    def generate_revenue_report_pdf(
        self, makerspace_id: str, start_date: date, end_date: date
    ) -> str:
        ...
        # Query revenue data
        revenue_data = (
            self.db.query(RevenueAnalytics)
            .filter(
        ...
        for item in revenue_data:
            revenue_by_type[item.revenue_type] = (
                revenue_by_type.get(item.revenue_type, 0) + item.amount
            )
            payment_method = item.payment_method or "Unknown"
            revenue_by_method[payment_method] = (
                revenue_by_method.get(payment_method, 0) + item.amount
            )
        ...
        summary_data = [
            ...
            [
                "Average Transaction",
                (
                    f"${total_revenue / len(revenue_data):.2f}"
                    if revenue_data
                    else "$0.00"
                ),
            ],
            [
                "Top Revenue Source",
                (
                    max(revenue_by_type.items(), key=lambda x: x[1])[0]
                    if revenue_by_type
                    else "N/A"
                ),
            ],
            [
                ...
                (
                    max(revenue_by_method.items(), key=lambda x: x[1])[0]
        ...
        breakdown_table = Table(
            breakdown_data, colWidths=[2 * inch, 1.5 * inch, 1.5 * inch]
        )
    ...
    def generate_equipment_report_xlsx(
        self, makerspace_id: str, start_date: date, end_date: date
    ) -> str:
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            ...
            for log in usage_logs:
                ...
                if log.success_rate is not None:
                    equipment_summary[eq_id]["avg_success_rate"].append(
                        log.success_rate
                    )
            ...
            pd.DataFrame(summary_data).to_excel(
                writer, sheet_name="Equipment Summary", index=False
            )
            ...
            pd.DataFrame(detailed_data).to_excel(
                writer, sheet_name="Usage Logs", index=False
            )

        return filepath

    def generate_projects_report_csv(
        self, makerspace_id: str, start_date: date, end_date: date
    ) -> str:
