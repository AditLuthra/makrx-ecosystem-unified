...
class EnhancedEquipmentReservation(Base):
    ...
    equipment_condition_before = Column(
    ...
    priority_level = Column(
        Integer, default=0
    )  # 0=normal, 1=high, 2=urgent, 3=emergency
...
class EquipmentSkillGate(Base):
    ...
    required_training_modules = Column(
        JSON, nullable=True
    )  # List of training module IDs
    ...
    allowed_hours = Column(
        JSON, nullable=True
    )  # Weekly schedule when skill gate applies
    restricted_hours = Column(
        JSON, nullable=True
    )  # Times when extra supervision needed

    # Grace period and override
    grace_period_days = Column(
        Integer, default=0
    )  # Days after cert expiry before blocking
...
class ReservationCostBreakdown(Base):
    ...
    reservation_id = Column(
        String,
        ForeignKey("enhanced_equipment_reservations.id"),
        nullable=False,
    )
    ...
    reservation = relationship(
        "EnhancedEquipmentReservation", back_populates="cost_breakdown"
    )
...
class ReservationSkillVerification(Base):
    ...
    skill_gate_id = Column(
        String, ForeignKey("equipment_skill_gates.id"), nullable=False
    )
    ...
    # Relationships
    reservation = relationship(
        "EnhancedEquipmentReservation", back_populates="skill_verifications"
    )
...
class ReservationConflictLog(Base):
    ...
    conflict_type = Column(
        String(50), nullable=False
    )  # double_booking, skill_gate, maintenance, etc.
    primary_reservation_id = Column(
        String, ForeignKey("enhanced_equipment_reservations.id"), nullable=True
    )
    conflicting_reservation_id = Column(
        String, ForeignKey("enhanced_equipment_reservations.id"), nullable=True
    )
    ...
    )  # cancelled, rescheduled, overridden, etc.
    ...
    # Relationships
    equipment = relationship("Equipment")
    primary_reservation = relationship(
        "EnhancedEquipmentReservation", foreign_keys=[primary_reservation_id]
    )
    conflicting_reservation = relationship(
        "EnhancedEquipmentReservation",
        foreign_keys=[conflicting_reservation_id],
    )
