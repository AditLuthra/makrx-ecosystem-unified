...
class UserCertification(Base):
    ...
    certification_method = Column(
        String(50), default="manual"
    )  # manual, quiz, practical_test
    ...
    access_attempts = relationship(
        "MachineAccessAttempt", back_populates="certification"
    )
...
class UserBadge(Base):
    ...
    )  # User who manually awarded (if applicable)

    # Context of earning
    earned_through = Column(
        String(100), nullable=True
    )  # "project_completion", "equipment_mastery", etc.
    related_entity_id = Column(
        String(100), nullable=True
    )  # Project ID, Equipment ID, etc.
...
class SafetyIncident(Base):
    ...
    certification_action = Column(
        String(100), nullable=True
    )  # "suspend", "revoke", "retrain"
...
class SkillAssessment(Base):
    ...
    )  # "initial", "renewal", "retest", "upgrade"
...
class AccessControlPolicy(Base):
    ...
    supervision_required_for = Column(
        JSON, nullable=True
    )  # List of equipment categories
    supervisor_certification_level = Column(
        Enum(AccessLevel), default=AccessLevel.ADVANCED
    )
