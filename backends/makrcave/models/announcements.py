...
class Announcement(Base):
    ...
    announcement_type = Column(
        SQLEnum(AnnouncementType), default=AnnouncementType.GENERAL
    )
    priority = Column(SQLEnum(Priority), default=Priority.NORMAL)

    # Targeting
    target_audience = Column(
        SQLEnum(TargetAudience), default=TargetAudience.ALL_MEMBERS
    )
    ...
    )  # When to publish (future scheduling)
    expires_at = Column(
        DateTime(timezone=True), nullable=True
    )  # When announcement expires
    ...
    require_acknowledgment = Column(
        Boolean, default=False
    )  # Members must acknowledge reading
    ...
    acknowledgments = relationship(
        "AnnouncementAcknowledgment", back_populates="announcement"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "target_audience": (
                self.target_audience.value if self.target_audience else None
            ),
    ...
    def is_targeted_to_member(self, member):
        ...
        if self.target_audience == TargetAudience.ADMINS_ONLY:
            return (
                member.role in ["admin", "makerspace_admin", "super_admin"]
