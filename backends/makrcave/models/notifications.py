...
class NotificationTemplate(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    makerspace_id = Column(
        UUID(as_uuid=True), ForeignKey("makerspaces.id"), nullable=True
    )  # Null for global
...
class NotificationRule(Base):
    ...
    throttle_duration_minutes = Column(
    ...
    )  # specific_users, role_based, all_members
    recipient_criteria = Column(
        JSON, nullable=True
...
class NotificationQueue(Base):
    ...
    process_after = Column(
        DateTime(timezone=True), nullable=True
    )  # Delay processing until this time
...
Index(
    "idx_notifications_type_scheduled",
    Notification.notification_type,
    Notification.scheduled_at,
)
Index(
    "idx_notifications_makerspace_created",
    Notification.makerspace_id,
    Notification.created_at,
)
Index(
    "idx_notification_queue_priority",
    NotificationQueue.queue_name,
    NotificationQueue.priority.desc(),
)
Index(
    "idx_notification_analytics_date",
    NotificationAnalytics.makerspace_id,
