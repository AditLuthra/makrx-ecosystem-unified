...
class UsageEvent(Base):
    ...
    )  # ID of equipment, project, etc.
    resource_type = Column(
        String(50), nullable=True
    )  # 'equipment', 'project', 'inventory'
...
class EquipmentUsageLog(Base):
    ...
    job_id = Column(
        UUID(as_uuid=True), nullable=True
    )  # Related print job if applicable
...
class InventoryAnalytics(Base):
    ...
    projects_using = Column(
        Integer, default=0
    )  # Number of projects that used this item
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class RevenueAnalytics(Base):
    ...
    revenue_type = Column(
        String(50), nullable=False
    )  # 'membership', 'credits', 'store', 'service'
