...
class FilamentRoll(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "deduction_method": (
                self.deduction_method.value if self.deduction_method else None
            ),
    ...
    def calculate_remaining_weight(self):
        ...
        self.remaining_weight_g = max(
            0, self.current_weight_g - (self.spool_weight_g or 0)
        )
    ...
    def calculate_usage_percentage(self):
        ...
        return (
            min(100, (self.used_weight_g / net_original) * 100)
            if net_original > 0
            else 0
        )
    ...
    def can_fulfill_print(self, required_weight_g: float):
        ...
        available_weight = self.remaining_weight_g - (
            self.remaining_weight_g * self.safety_buffer_percentage / 100
        )
...
class FilamentUsageLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    filament_roll_id = Column(
        UUID(as_uuid=True), ForeignKey("filament_rolls.id"), nullable=False
    )
...
class FilamentReorderRequest(Base):
    ...
    trigger_weight_g = Column(
        Float, nullable=True
    )  # Weight that triggered auto-reorder
