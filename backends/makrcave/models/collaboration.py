...
class UserPresence(Base):
    ...
    current_view = Column(
        String
    )  # overview, team, bom, equipment, files, activity, whiteboard, documents
...
class CollaborationSession(Base):
    ...
    )  # video_call, screen_share, whiteboard_session
...
def add_collaboration_relationships():
    ...
    if not hasattr(Project, "collaboration_messages"):
        Project.collaboration_messages = relationship(
            "CollaborationMessage", back_populates="project"
        )

    if not hasattr(Project, "whiteboard_actions"):
        Project.whiteboard_actions = relationship(
            "WhiteboardAction", back_populates="project"
        )

    if not hasattr(Project, "document_versions"):
        Project.document_versions = relationship(
            "DocumentVersion", back_populates="project"
        )
    ...
    if not hasattr(Project, "collaboration_sessions"):
        Project.collaboration_sessions = relationship(
            "CollaborationSession", back_populates="project"
        )

    if not hasattr(Project, "project_comments"):
        Project.project_comments = relationship(
