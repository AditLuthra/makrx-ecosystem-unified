...
class Member(Base):
    ...
    suspended_by_user = relationship(
        "Member", remote_side=[id], foreign_keys=[suspended_by]
    )
    ...
    def enable_two_factor(
        self, method: TwoFactorMethod, secret: str = None, phone: str = None
    ):
    ...
    def can_access_makerspace(self) -> bool:
        ...
        return (
            self.is_active
            and self.status == MemberStatus.ACTIVE
            and not self.is_account_locked()
            and not self.is_membership_expired()
        )
        ...
        for role in self.roles:
            if role.menu_restrictions:
                if "hidden_items" in role.menu_restrictions:
                    restrictions["hidden_items"].extend(
                        role.menu_restrictions["hidden_items"]
                    )
                if "disabled_items" in role.menu_restrictions:
                    restrictions["disabled_items"].extend(
                        role.menu_restrictions["disabled_items"]
                    )
            ...
            data.update(
                {
                    ...
                    "password_expires_at": (
                        self.password_expires_at.isoformat()
                    ...
                    "terms_accepted_at": (
                        self.terms_accepted_at.isoformat()
                    ...
                    "gdpr_consent_date": (
                        self.gdpr_consent_date.isoformat()
