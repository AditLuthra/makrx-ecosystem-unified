...
class Role(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "parent_role_id": (
                str(self.parent_role_id) if self.parent_role_id else None
            ),
            ...
            "required_membership_plans": (
                [str(plan_id) for plan_id in self.required_membership_plans]
                if self.required_membership_plans
                else []
            ),
            "excluded_membership_plans": (
                [str(plan_id) for plan_id in self.excluded_membership_plans]
...
class AccessLog(Base):
    ...
    )  # login, logout, view_page, api_call, etc.
...
class PasswordPolicy(Base):
    ...
    progressive_lockout = Column(
        Boolean, default=True
    )  # Increase lockout time with each attempt
...
def create_default_roles():
    ...
    return [
        ...
        },
        {
            ...
            "is_default": True,
            "priority_level": 100,
            "permissions": [PermissionType.VIEW_EQUIPMENT.value],
        },
        {
