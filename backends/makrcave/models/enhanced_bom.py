...
class EnhancedBOMItem(Base):
    ...
    procurement_status = Column(
        Enum(ProcurementStatus), default=ProcurementStatus.NEEDED
    )
    availability_status = Column(
        Enum(AvailabilityStatus), default=AvailabilityStatus.UNKNOWN
    )
...
class BOMPurchaseOrder(Base):
    ...
    id = Column(String(100), primary_key=True, index=True)
    bom_item_id = Column(
        String(100), ForeignKey("enhanced_bom_items.id"), nullable=False
    )
...
class BOMUsageHistory(Base):
    ...
    )  # Assembly, testing, prototyping, etc.
    batch_number = Column(String(100), nullable=True)
    serial_numbers = Column(JSON, nullable=True)  # For serialized components

    # Quality and outcome
    usage_outcome = Column(
        String(50), default="successful"
    )  # successful, failed, returned
...
class BOMInventoryTransaction(Base):
    ...
    )  # purchase, usage, adjustment, transfer, return
