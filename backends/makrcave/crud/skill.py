...
def create_skill(db: Session, skill: SkillCreate) -> Skill:
    ...
    if skill.prerequisite_ids:
        prerequisites = (
            db.query(Skill).filter(Skill.id.in_(skill.prerequisite_ids)).all()
        )
...
def get_skill(db: Session, skill_id: str) -> Optional[Skill]:
    return (
        db.query(Skill)
        .options(joinedload(Skill.prerequisites), joinedload(Skill.equipment))
...
def update_skill(
    db: Session, skill_id: str, skill_update: SkillUpdate
) -> Optional[Skill]:
...
def delete_skill(db: Session, skill_id: str) -> bool:
    ...
    user_skills_count = (
        db.query(UserSkill).filter(UserSkill.skill_id == skill_id).count()
    )
...
def update_user_skill(
    ...
    if (
        user_skill_update.status == SkillStatus.CERTIFIED
        and old_status != SkillStatus.CERTIFIED
    ):
        db_user_skill.certified_at = datetime.utcnow()
...
def create_skill_request(
    db: Session, skill_request: SkillRequestCreate, user_id: str
) -> SkillRequest:
...
def check_equipment_access(
    db: Session, user_id: str, equipment_id: str
) -> EquipmentAccessCheck:
    ...
    equipment = (
        db.query(Equipment)
        .options(joinedload(Equipment.required_skills))
    ...
    user_skills = (
        ...
        .filter(
            and_(
                ...
                or_(
                    UserSkill.expires_at.is_(None),
                    UserSkill.expires_at > datetime.utcnow(),
                ),
            )
        )
        .all()
...
def get_user_accessible_equipment(
    db: Session, user_id: str, makerspace_id: Optional[str] = None
) -> List[str]:
    from models.equipment import Equipment
...
def get_skill_analytics(db: Session, makerspace_id: str) -> Dict[str, Any]:
    ...
    total_user_skills = (
        db.query(UserSkill)
        .join(Skill)
        .filter(Skill.makerspace_id == makerspace_id)
        .count()
    )

    certified_user_skills = (
        db.query(UserSkill)
        .join(Skill)
        .filter(
            and_(
