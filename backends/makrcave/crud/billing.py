...
def update_transaction(
    db: Session, transaction_id: str, update: TransactionUpdate
) -> Optional[Transaction]:
...
def update_invoice(
    db: Session, invoice_id: str, update: InvoiceUpdate
) -> Optional[Invoice]:
    """Update an invoice"""
...
def get_or_create_credit_wallet(
    db: Session, user_id: str, makerspace_id: str
) -> CreditWallet:
    ...
    wallet = (
        ...
        .filter(
            and_(
                ...
                CreditWallet.makerspace_id == makerspace_id,
            )
        )
        .first()
    )
...
def create_credit_transaction(
    db: Session, credit_transaction: CreditTransactionCreate
) -> CreditTransaction:
    """Create a credit transaction and update wallet balance"""
    wallet = (
        db.query(CreditWallet)
        .filter(CreditWallet.id == credit_transaction.wallet_id)
        .first()
    )
    if not wallet:
...
def update_credit_wallet_from_transaction(db: Session, transaction: Transaction):
    ...
    wallet = get_or_create_credit_wallet(
        db, transaction.user_id, transaction.makerspace_id
    )
...
def get_credit_transactions(
    db: Session, wallet_id: str, skip: int = 0, limit: int = 100
) -> List[CreditTransaction]:
    """Get credit transactions for a wallet"""
    return (
        db.query(CreditTransaction)
        .filter(CreditTransaction.wallet_id == wallet_id)
...
def get_billing_analytics(
    ...
    revenue_this_month = (
        base_query.filter(Transaction.created_at >= current_month_start)
    ...
    top_services = (
        base_query.filter(Transaction.service_type.isnot(None))
        .with_entities(
    ...
    top_services_data = [
        {"service": service, "revenue": float(revenue), "transactions": count}
