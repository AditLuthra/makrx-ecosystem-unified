...
def get_member_by_keycloak_id(db: Session, keycloak_user_id: str) -> Optional[Member]:
    ...
    return (
        db.query(Member)
        .options(joinedload(Member.membership_plan))
...
def update_member(
    db: Session, member_id: str, member_update: MemberUpdate, updated_by: str
) -> Optional[Member]:
    """Update a member"""
...
def suspend_member(
    db: Session, member_id: str, suspend_data: MemberSuspend
) -> Optional[Member]:
    """Suspend a member"""
    db_member = get_member(db, member_id)
    if not db_member:
...
def reactivate_member(
    db: Session, member_id: str, reactivated_by: str
) -> Optional[Member]:
...
def get_membership_plans(
    db: Session, makerspace_id: str, active_only: bool = True
) -> List[MembershipPlan]:
...
def update_membership_plan(
    db: Session, plan_id: str, plan_update: MembershipPlanUpdate
...
def get_member_invite(db: Session, invite_id: str) -> Optional[MemberInvite]:
    ...
    return (
        ...
        .options(joinedload(MemberInvite.membership_plan))
        .filter(MemberInvite.id == invite_id)
        .first()
    )


def get_member_invite_by_token(
    db: Session, invite_token: str
) -> Optional[MemberInvite]:
    """Get a member invite by token"""
    return (
        db.query(MemberInvite)
        .options(joinedload(MemberInvite.membership_plan))
        .filter(MemberInvite.invite_token == invite_token)
        .first()
    )


def get_member_invites(
    db: Session, makerspace_id: str, status: Optional[InviteStatus] = None
) -> List[MemberInvite]:
    """Get member invites for a makerspace"""
    query = (
        db.query(MemberInvite)
        .options(joinedload(MemberInvite.membership_plan))
        .filter(MemberInvite.makerspace_id == makerspace_id)
    )
...
def update_member_invite(
    db: Session, invite_id: str, invite_update: MemberInviteUpdate
) -> Optional[MemberInvite]:
...
def accept_member_invite(
    db: Session, invite_token: str, keycloak_user_id: str
) -> Optional[Member]:
    ...
    if (
        ...
        or db_invite.expires_at < datetime.utcnow()
    ):
        return None
...
def get_member_activities(
    db: Session, member_id: str, limit: int = 50
) -> List[MemberActivityLog]:
    """Get member activities"""
    return (
        db.query(MemberActivityLog)
        .filter(MemberActivityLog.member_id == member_id)
        .order_by(desc(MemberActivityLog.created_at))
        .limit(limit)
        .all()
    )
...
def get_member_statistics(db: Session, makerspace_id: str) -> Dict[str, Any]:
    ...
    total_members = (
        db.query(Member).filter(Member.makerspace_id == makerspace_id).count()
    )
...
def check_expired_memberships(db: Session) -> List[Member]:
    ...
    for member in expired_members:
        ...
        # Log activity
        log_member_activity(
            db, member.id, "membership_expired", "Membership has expired"
        )
