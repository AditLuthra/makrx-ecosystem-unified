...
def create_permission(
    db: Session, permission: PermissionCreate, created_by: str
) -> Permission:
    """Create a new permission"""
    db_permission = Permission(**permission.dict(), created_by=created_by)
...
def update_permission(
    db: Session, permission_id: str, permission_update: PermissionUpdate
) -> Optional[Permission]:
    """Update permission"""
...
def update_role(
    db: Session, role_id: str, role_update: RoleUpdate, updated_by: str
) -> Optional[Role]:
    """Update role"""
...
def assign_role_to_user(
    db: Session, assignment: RoleAssignmentCreate, assigned_by: str
) -> RoleAccessLog:
    """Assign role to user"""
    # Get user and role
...
def terminate_session(
    db: Session, session_id: str, reason: str = "user_logout"
) -> bool:
...
def get_audit_logs(
    db: Session, filters: AccessControlFilter, skip: int = 0, limit: int = 100
) -> List[AccessLog]:
...
def create_password_policy(
    db: Session, policy: PasswordPolicyCreate, created_by: str
) -> PasswordPolicy:
    """Create password policy"""
    db_policy = PasswordPolicy(**policy.dict(), created_by=created_by)
    db.add(db_policy)
    db.commit()
...
def get_access_control_stats(db: Session, makerspace_id: Optional[str] = None) -> dict:
    ...
    if makerspace_id:
        ...
        session_query = session_query.join(Member).filter(
...
def get_security_alerts(
    ...
    recent_failures = (
        ...
        .having(func.count(AccessLog.id) >= 5)
        .all()
    )
...
def setup_default_roles_and_permissions(db: Session, makerspace_id: str) -> dict:
    ...
    for role_data in default_roles:
        ...
        if not existing:
            ...
            permissions = (
                db.query(Permission)
                .filter(Permission.codename.in_(permission_codes))
                .all()
            )
