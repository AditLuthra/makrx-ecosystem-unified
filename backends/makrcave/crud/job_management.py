...
def get_service_jobs(
    ...
    if filters:
        ...
        if filters.assigned_provider_id:
            query = query.filter(
                ServiceJob.assigned_provider_id == filters.assigned_provider_id
            )
        ...
        if filters.customer_email:
            query = query.filter(
                ServiceJob.customer_email.ilike(f"%{filters.customer_email}%")
            )
...
def create_service_job(
    db: Session, job: ServiceJobCreate, job_id: str, customer_id: str
) -> ServiceJob:
...
def update_service_job(
    db: Session, job_id: str, job_update: ServiceJobUpdate
) -> Optional[ServiceJob]:
...
def get_job_files(db: Session, job_id: str) -> List[ServiceJobFile]:
    ...
    return (
        db.query(ServiceJobFile)
        .filter(ServiceJobFile.job_id == job_id)
        .order_by(desc(ServiceJobFile.uploaded_at))
        .all()
    )


def create_job_file(
    db: Session, job_id: str, file_data: ServiceJobFileUpload, uploaded_by: str
) -> ServiceJobFile:
...
# Status Update CRUD
def get_job_status_updates(
    db: Session, job_id: str, customer_visible_only: bool = False
...
def get_job_material_usage(db: Session, job_id: str) -> List[JobMaterialUsage]:
    ...
    return (
        db.query(JobMaterialUsage)
        .filter(JobMaterialUsage.job_id == job_id)
        .order_by(desc(JobMaterialUsage.recorded_at))
        .all()
    )
...
def get_service_provider(db: Session, provider_id: str) -> Optional[ServiceProvider]:
    ...
    return (
        ...
        .filter(ServiceProvider.provider_id == provider_id)
        .first()
    )


def get_service_provider_by_user(
...
def update_service_provider(
    db: Session, provider_id: str, provider_update: ServiceProviderUpdate
) -> Optional[ServiceProvider]:
...
def get_provider_equipment(db: Session, provider_id: str) -> List[ProviderEquipment]:
    ...
    return (
        ...
        .filter(ProviderEquipment.provider_id == provider_id)
        .order_by(desc(ProviderEquipment.created_at))
        .all()
    )


def create_provider_equipment(
    db: Session, provider_id: str, equipment: ProviderEquipmentCreate
) -> ProviderEquipment:
...
def get_job_dashboard_stats(
    db: Session, user_id: Optional[str] = None, user_role: Optional[str] = None
) -> Dict[str, Any]:
    ...
    if completed_jobs:
        total_time = sum(
            [
                (job.actual_completion - job.actual_start).total_seconds()
                / 86400  # days
    ...
    for material_type in FilamentType:
        usage = (
            db.query(func.sum(JobMaterialUsage.actual_weight))
            .filter(
                and_(
    ...
    revenue_today = (
        db.query(func.sum(ServiceJob.final_price))
        .filter(
            and_(
                ServiceJob.status == JobStatus.COMPLETED,
                func.date(ServiceJob.actual_completion) == today,
...
def get_provider_stats(db: Session, provider_id: str) -> Dict[str, Any]:
    ...
    provider_query = db.query(ServiceJob).filter(
    ...
    # Average job value
    completed_jobs = provider_query.filter(
        ServiceJob.status == JobStatus.COMPLETED
    ).all()
    ...
    on_time_jobs = [
        job
        for job in completed_jobs
        if job.deadline and job.actual_completion <= job.deadline
    ]
