...
def update_project(
    db: Session, project_id: str, project_update: ProjectUpdate, user_id: str
) -> Optional[Project]:
...
def remove_collaborator(
    db: Session, project_id: str, user_id: str, removed_by: str
) -> bool:
...
# BOM CRUD operations
def add_bom_item(
    db: Session, project_id: str, bom_item: BOMItemCreate, added_by: str
) -> Optional[ProjectBOM]:
    ...
    db_bom_item = ProjectBOM(
        ...
        alternatives=(
            [alt.dict() for alt in bom_item.alternatives]
            if bom_item.alternatives
            else []
        ),
...
def add_milestone(
    db: Session, project_id: str, milestone: MilestoneCreate, created_by: str
    ...
    max_order = (
        db.query(func.max(ProjectMilestone.order_index))
...
def complete_milestone(
    db: Session, milestone_id: int, completed_by: str
) -> Optional[ProjectMilestone]:
...
def disconnect_github_repo(
    ...
    if user_id:
        activity_log = ProjectActivityLog(
            ...
            user_id=user_id,
            user_name="User",
            metadata={"repo_name": old_repo_name},
        )
        db.add(activity_log)
...
def sync_github_activity(
    ...
    try:
        ...
        pull_requests = github_service.get_pull_requests(
            project.github_repo_url, per_page=min(limit // 3, 10)
        )
            ...
            existing = (
                ...
                .filter(
                    and_(
                        ...
                        ProjectActivityLog.metadata["pr_number"].astext
                        == str(pr.metadata["number"]),
                    )
                )
                .first()
            )
        ...
        for issue in issues:
            activity_type = (
                ActivityType.GITHUB_ISSUE_CREATED
                if issue.action == "open"
                else ActivityType.GITHUB_ISSUE_CLOSED
            )
            ...
            existing = (
                ...
                .filter(
                    and_(
                        ProjectActivityLog.project_id == project_id,
                        ProjectActivityLog.activity_type == activity_type,
                        ProjectActivityLog.metadata["issue_number"].astext
                        == str(issue.metadata["number"]),
                    )
                )
                .first()
            )
