...
class EquipmentCRUD:
    def get_equipment_list(
        ...
        if filters.makerspace_id and user_role == "super_admin":
            query = query.filter(
                Equipment.linked_makerspace_id == filters.makerspace_id
            )
        ...
        if filters.sort_order == "desc":
            query = query.order_by(
                desc(getattr(Equipment, filters.sort_by, Equipment.name))
            )
        else:
            query = query.order_by(
                asc(getattr(Equipment, filters.sort_by, Equipment.name))
            )
    ...
    def get_equipment_by_id(
        self, db: Session, equipment_id: str
    ) -> Optional[Equipment]:
        """Get equipment by ID with all relationships"""
        return db.query(Equipment).filter(Equipment.id == equipment_id).first()

    def get_equipment_by_equipment_id(
        self, db: Session, equipment_id: str
    ) -> Optional[Equipment]:
    ...
    def create_reservation(
        ...
        # Calculate duration
        duration = (
            reservation_data.end_time - reservation_data.start_time
        ).total_seconds() / 3600
    ...
    def update_reservation(
        ...
        db_reservation = (
            db.query(EquipmentReservation)
            .filter(EquipmentReservation.id == reservation_id)
            .first()
        )
    ...
    def update_maintenance_log(
        ...
        db_maintenance = (
            ...
            .filter(EquipmentMaintenanceLog.id == maintenance_id)
            .first()
        )
    ...
    def create_rating(
        ...
        if equipment:
            total_rating = (
                equipment.average_rating * equipment.total_ratings
    ...
    def get_equipment_stats(
        ...
        in_use_equipment = query.filter(
            Equipment.status == EquipmentStatus.IN_USE
        ).count()
        maintenance_equipment = query.filter(
            Equipment.status == EquipmentStatus.UNDER_MAINTENANCE
        ).count()
        offline_equipment = query.filter(
        ...
        reservations_today = (
            db.query(EquipmentReservation)
            .filter(func.date(EquipmentReservation.start_time) == today)
            .count()
        )
    ...
    def get_equipment_availability(
        self, db: Session, equipment_id: str, date: datetime
    ) -> Dict[str, Any]:
