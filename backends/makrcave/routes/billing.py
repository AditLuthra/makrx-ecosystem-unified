...
@router.get("/usage-by-category", response_model=List[dict])
async def get_usage_by_category(
    ...
    results = (
        ...
        .group_by(Transaction.service_type)
        .all()
    )
    # Assign colors (static or dynamic)
    color_map = {
        "printing_3d": "#8884d8",
        "laser_cut": "#82ca9d",
        "workshop": "#ffc658",
...
async def get_transactions(
    ...
    filters = TransactionFilter(
        user_id=(
            user_id
            if _can_view_all_transactions(current_user)
    ...
    transactions = crud_billing.get_transactions(
...
async def update_transaction(
    ...
    transaction = crud_billing.update_transaction(
...
async def list_payment_methods(
    ...
    """List active payment methods for the current user"""
    try:
        return crud_billing.get_payment_methods(db, current_user.user_id)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_payment_method(
    ...
    # Enforce ownership to current user
    method.user_id = current_user.user_id
    try:
        return crud_billing.create_payment_method(db, method)
    except Exception as e:
        raise HTTPException(
...
async def set_default_payment_method(
    ...
    success = crud_billing.set_default_payment_method(
...
async def verify_payment(
    ...
    try:
        ...
        transaction = (
            ...
            .filter(
                crud_billing.Transaction.gateway_order_id
        ...
        # Verify payment
        is_valid, payment_details = payment_service.verify_payment(
            "razorpay", payment_data
        )
        ...
        if is_valid:
            ...
            if background_tasks:
                background_tasks.add_task(
                    generate_and_send_invoice, db, completed_transaction.id
                )
            ...
            crud_billing.fail_transaction(
                db, transaction.id, "Payment verification failed"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_credit_wallet(
    ...
    makerspace_id = _get_user_makerspace_id(current_user)
    wallet = crud_billing.get_or_create_credit_wallet(
...
async def get_invoices(
    ...
    invoices = crud_billing.get_invoices(
        db, user_id, makerspace_id, status_filter, skip, limit
    )
    return invoices
...
async def charge_for_service(
    ...
    try:
        ...
        if service_billing.auto_charge:
            ...
            if crud_billing.can_use_credits(
                db,
                ...
                if success:
                    ...
                    db_transaction = crud_billing.create_transaction(
                        db, transaction_data
                    )
...
async def get_billing_analytics(
    ...
    makerspace_id = _get_user_makerspace_id(current_user)
    analytics = crud_billing.get_billing_analytics(
        db, makerspace_id, start_date, end_date
    )
    ...
    try:
        ...
        if event_type == "payment.captured":
            ...
            if order_id:
                ...
                if transaction:
                    crud_billing.complete_transaction(
                        db, transaction.id, payment_data.get("id"), order_id
                    )
...
async def generate_and_send_invoice(db: Session, transaction_id: str):
    ...
    try:
        ...
        file_path = generate_invoice_filename(
            invoice.invoice_number, invoice.makerspace_id
        )
        save_invoice_to_file(invoice_dict, file_path)

        # Update invoice with file path
        crud_billing.update_invoice(
            db, invoice.id, InvoiceUpdate(file_path=file_path, status="sent")
        )
