...
async def get_usage_by_category(
    ...
    results = (
        db.query(Transaction.service_type, func.sum(Transaction.amount).label("value"))
        .filter(
    ...
    data = [
        {
            "name": service_type.replace("_", " ").title() if service_type else "Other",
            "value": float(value),
...
async def list_payment_methods(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to load payment methods: {str(e)}")
...
async def create_payment_method(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Failed to create payment method: {str(e)}")
...
async def set_default_payment_method(
    ...
    success = crud_billing.set_default_payment_method(db, current_user.user_id, method_id)
    if not success:
