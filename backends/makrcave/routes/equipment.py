...
from models.equipment import EquipmentStatus, EquipmentCategory, Equipment, EquipmentReservation
from schemas.equipment import (
    EquipmentResponse, EquipmentCreate, EquipmentUpdate,
    EquipmentReservationCreate, EquipmentReservationResponse,
    EquipmentRatingCreate, EquipmentRatingResponse
)
...
async def create_equipment(
    ...
    try:
        ...
        return {"message": "Equipment created successfully", "equipment_id": new_equipment.id}
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reserve_equipment(
    ...
    try:
        ...
        return {"message": "Equipment reserved successfully", "reservation_id": reservation.id}
    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post("/{equipment_id}/rating", response_model=EquipmentRatingResponse, status_code=status.HTTP_201_CREATED)
async def create_equipment_rating(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create rating: {str(e)}")
