...
# Schemas
from schemas.project_showcase import ShowcaseProjectResponse, ShowcaseBOMItem, ProjectAward
# Avoid importing member model here to prevent duplication; not used directly
...
async def get_showcase_projects(
    ...
    for project in projects:
        ...
        owner = db.query(Member).filter(Member.user_id == project.owner_id).first() if hasattr(project, 'owner_id') else None
        # Get collaborator count
            from sqlalchemy.future import select
            showcase_projects = []
            # Build base query for public projects
            stmt = select(Project).where(Project.visibility == 'public', Project.is_approved == True)
            # Apply filters
            if category:
            ...
            if sort_by == "newest":
                ...
                stmt = stmt.order_by((Project.like_count + Project.view_count + Project.fork_count).desc(), Project.updated_at.desc())
            # Apply pagination
            ...
            for project in projects:
                try:
                    # Get project owner info
                    owner_result = await db.execute(select(Member).where(Member.id == project.owner_id))
                    owner = owner_result.scalars().first() if hasattr(project, 'owner_id') else None
                    # Get collaborator count
                    collab_result = await db.execute(select(ProjectCollaborator).where(ProjectCollaborator.project_id == project.project_id))
                    collaborator_count = len(collab_result.scalars().all()) if hasattr(project, 'project_id') else 0
                    # Calculate completion rate (mock calculation)
                    completion_rate = min(95, max(10, 50 + (getattr(project, 'like_count', 0) * 2)))
                    ...
                    skills_required = getattr(project, 'required_skills', None) or ["3D Printing", "Electronics", "Programming"][:3]
                    ...
                    if getattr(project, 'is_featured', False):
                        awards.append({
                            ...
                            "icon": "⭐",
                            "awarded_at": project.featured_at.isoformat() if getattr(project, 'featured_at', None) else datetime.utcnow().isoformat()
                        })
                    is_liked = bool(await get_project_like(db, project.id, current_user["user_id"]))
                    is_bookmarked = bool(await get_project_bookmark(db, project.id, current_user["user_id"]))
                    is_following_owner = bool(owner and await get_project_follow(db, str(owner.id), current_user["user_id"]))
                    showcase_project = ShowcaseProjectResponse(
                        project_id=project.project_id,
                        name=project.name,
                        description=project.description,
                        owner_id=project.owner_id,
                        owner_name=f"{owner.first_name} {owner.last_name}" if owner else "Unknown Maker",
                        ...
                        featured_at=project.featured_at.isoformat() if getattr(project, 'featured_at', None) else None,
                        is_liked=is_liked,
                    ...
                    import logging
                    logging.warning(f"Error processing project {getattr(project, 'id', None)}: {e}")
                    continue
...
async def get_showcase_filters(
    ...
    categories_stmt = select(Project.category).where(Project.visibility == 'public', Project.category.isnot(None)).distinct()
    categories_result = await db.execute(categories_stmt)
    categories = [cat for cat in categories_result.scalars().all() if cat]
    tags_stmt = select(Project.tags).where(Project.visibility == 'public', Project.tags.isnot(None))
    ...
    unique_tags = list(set(all_tags))
    skills_stmt = select(Project.required_skills).where(Project.visibility == 'public', Project.required_skills.isnot(None))
...
async def get_trending_projects(
    ...
    async def get_featured_projects(
        ...
        stmt = select(Project).where(
            ...
            Project.is_approved == True
        ).order_by(Project.featured_at.desc()).limit(limit)
        result = await db.execute(stmt)
        ...
        for project in featured_projects:
            ...
            showcase_project = ShowcaseProjectResponse(
                ...
                subcategories=getattr(project, 'subcategories', []),
                tags=getattr(project, 'tags', []),
                skills_required=getattr(project, 'required_skills', ["3D Printing", "Electronics"]),
                ...
                is_staff_pick=getattr(project, 'is_staff_pick', False),
                is_trending=getattr(project, 'is_trending', False),
                awards=[ProjectAward(type="featured", name="Featured Project", icon="⭐", awarded_at=project.featured_at.isoformat() if project.featured_at else datetime.utcnow().isoformat())],
                created_at=project.created_at.isoformat(),
                updated_at=project.updated_at.isoformat(),
                featured_at=project.featured_at.isoformat() if project.featured_at else None,
                is_liked=False,
