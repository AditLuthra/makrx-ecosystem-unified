...
@router.post(
    "/",
    response_model=EnhancedReservationResponse,
...
async def create_enhanced_reservation(
    ...
    try:
        ...
        equipment = (
            ...
            .filter(models.Equipment.id == reservation_data.equipment_id)
            .first()
        )
        ...
        blocking_gates = [
            ...
            if not gate["passed"] and gate["enforcement_level"] == "block"
        ]
        ...
        # Calculate duration
        duration = (
            reservation_data.requested_end - reservation_data.requested_start
        ).total_seconds() / 3600
        ...
        if (
            not blocking_gates
            and not supervisor_gates
            and not reservation_data.is_emergency
            and total_cost < 100
        ):  # Auto-approve low-cost reservations
        ...
        if background_tasks:
            background_tasks.add_task(
                send_reservation_notifications, db_reservation, "created"
            )
...
async def list_reservations(
    ...
    if current_user["role"] not in ["super_admin", "makerspace_admin"]:
        ...
        query = query.filter(
            models.EnhancedEquipmentReservation.user_id == current_user.user_id
        )
    ...
    if start_date:
        query = query.filter(
            models.EnhancedEquipmentReservation.requested_start >= start_date
        )

    if end_date:
        query = query.filter(
            models.EnhancedEquipmentReservation.requested_end <= end_date
        )
...
async def get_reservation(
    ...
    reservation = (
        db.query(models.EnhancedEquipmentReservation)
        .filter(models.EnhancedEquipmentReservation.id == reservation_id)
        .first()
    )
    ...
    if (
        ...
        and reservation.user_id != current_user["user_id"]
    ):
        raise HTTPException(
...
async def update_reservation(
    ...
    # Check permissions
    can_edit = current_user["role"] in ["super_admin", "makerspace_admin"] or (
    ...
    if any(
        ...
        equipment = (
            db.query(models.Equipment)
            .filter(models.Equipment.id == reservation.equipment_id)
            .first()
        )
...
@router.post(
    "/cost-rules/",
    response_model=CostRuleResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_cost_rule(
    ...
    equipment = (
        db.query(models.Equipment)
        .filter(models.Equipment.id == rule_data.equipment_id)
        .first()
    )
...
@router.post(
    "/skill-gates/",
    response_model=SkillGateResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_skill_gate(
    ...
    equipment = (
        db.query(models.Equipment)
        .filter(models.Equipment.id == gate_data.equipment_id)
        .first()
    )
...
async def estimate_reservation_cost(
    ...
    equipment = (
        db.query(models.Equipment)
        .filter(models.Equipment.id == estimate_request.equipment_id)
        .first()
    )
    ...
    duration = (
        estimate_request.requested_end - estimate_request.requested_start
    ).total_seconds() / 3600
    ...
    user_data = {
        "user_id": estimate_request.user_id,
        "membership_tier": estimate_request.membership_tier,
    }
    cost_breakdown = await calculate_reservation_cost(
        db, equipment, temp_reservation, user_data
    )
...
async def check_availability(
    ...
    equipment = (
        db.query(models.Equipment)
        .filter(models.Equipment.id == availability_request.equipment_id)
        .first()
    )
    ...
    existing_reservations = (
        db.query(models.EnhancedEquipmentReservation)
        .filter(
            models.EnhancedEquipmentReservation.equipment_id
            == availability_request.equipment_id,
            ...
            models.EnhancedEquipmentReservation.requested_start
            < availability_request.end_date,
            models.EnhancedEquipmentReservation.requested_end
    ...
    if availability_request.user_id:
        skill_gates = await verify_skill_gates(
            db, availability_request.equipment_id, availability_request.user_id
        )
        skill_gate_blocking = [
            gate["gate_name"] for gate in skill_gates if not gate["passed"]
        ]
...
async def check_equipment_availability(
    db: Session, equipment_id: str, start_time: datetime, end_time: datetime
) -> Optional[str]:
...
async def verify_skill_gates(
    db: Session, equipment_id: str, user_id: str
) -> List[dict]:
...
async def calculate_reservation_cost(
    ...
    reservation: models.EnhancedEquipmentReservation,
    user_data: dict,
) -> dict:
...
async def send_reservation_notifications(
    reservation: models.EnhancedEquipmentReservation, action: str
):
