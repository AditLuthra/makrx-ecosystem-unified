...
async def create_membership_plan(
    ...
    if (
        plan_data.access_type == AccessType.TIME_LIMITED
        and not plan_data.max_hours_per_cycle
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def update_membership_plan(
    ...
    max_hours = (
        plan_data.max_hours_per_cycle
        if plan_data.max_hours_per_cycle is not None
        else plan.max_hours_per_cycle
    )
...
async def duplicate_membership_plan(
    ...
    new_plan = MembershipPlan(
        **plan_data, created_by=current_user.id, makerspace_id=makerspace_id
    )
...
async def initialize_default_plans(
    ...
    existing_count = (
        db.query(MembershipPlan)
        .filter(MembershipPlan.makerspace_id == makerspace_id)
        .count()
    )
    ...
    for plan_data in default_plans_data:
        plan = MembershipPlan(**plan_data, created_by=current_user.id)
        db.add(plan)
