...
async def get_skills(
    ...
    for skill in skills:
        user_count = len(
            skill_crud.get_user_skills(
                db=db, skill_id=skill.id, status=SkillStatus.CERTIFIED
            )
        )
...
async def grant_user_skill(
    ...
    return skill_crud.create_user_skill(
        db=db, user_skill=user_skill, certified_by=current_user.id
    )
...
async def get_user_skills(
    ...
    if not user_id and current_user.role not in [
        "super_admin",
        "makerspace_admin",
    ]:
...
async def create_skill_request(
    ...
    return skill_crud.create_skill_request(
        db=db, skill_request=skill_request, user_id=current_user.id
    )
...
async def check_equipment_access(
    ...
    return skill_crud.check_equipment_access(
        db=db, user_id=check_user_id, equipment_id=equipment_id
    )
...
async def check_bulk_equipment_access(
    ...
    for equipment in equipment_list:
        access_check = skill_crud.check_equipment_access(
            db=db, user_id=check_user_id, equipment_id=equipment.id
        )
...
@router.get(
    "/analytics/makerspace/{makerspace_id}",
    response_model=MakerspaceSkillOverview,
)
async def get_makerspace_skill_analytics(
    ...
    for skill in skills:
        ...
        skill_distribution[skill.category] = (
            skill_distribution.get(skill.category, 0) + 1
        )

        # Level distribution
        level_distribution[skill.level.value] = (
            level_distribution.get(skill.level.value, 0) + 1
        )
        ...
        pending_requests = len(
            [r for r in requests if r.status == RequestStatus.PENDING]
        )
        certified_users = len(
            [us for us in user_skills if us.status == SkillStatus.CERTIFIED]
        )
        approved_requests = len(
            [r for r in requests if r.status == RequestStatus.APPROVED]
        )
