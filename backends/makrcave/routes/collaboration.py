...
async def send_message(
    ...
    db_message = CollaborationMessage(
        ...
        message=InputSanitizer.sanitize_text(message.message),
...
async def broadcast_document_change(
    change: DocumentChangeCreate, current_user=Depends(get_current_user)
):
...
async def broadcast_cursor_position(
    cursor: CursorUpdate, current_user=Depends(get_current_user)
):
...
async def broadcast_whiteboard_cursor(
    cursor_data: dict, current_user=Depends(get_current_user)
):
    """Broadcast whiteboard cursor position"""

    await collaboration_manager.broadcast_to_project(
        cursor_data["project_id"],
        {
...
async def broadcast_editing_status(
    editing: EditingStatus, current_user=Depends(get_current_user)
):
...
async def broadcast_viewing_status(
    viewing_data: dict, current_user=Depends(get_current_user)
):
    """Broadcast viewing status to collaborators"""

    await collaboration_manager.broadcast_to_project(
        viewing_data["project_id"],
        {
    ...
    while True:
        try:
            ...
            for (
                user_id,
                presence,
            ) in collaboration_manager.user_presence.items():
            ...
            for user_id in inactive_users:
                if user_id in collaboration_manager.user_presence:
                    project_id = collaboration_manager.user_presence[user_id][
                        "project_id"
                    ]
