...
async def request_machine_access(
    ...
    if not access_rules:
        ...
        return MachineAccessResponse(
            ...
            + timedelta(hours=access_request.session_duration_hours or 4),
        )
...
async def end_machine_session(
    ...
    if access_attempt.session_start:
        duration = (
            access_attempt.session_end - access_attempt.session_start
        ).total_seconds() / 60
    ...
    if access_attempt.certification_id:
        certification = (
            db.query(UserCertification)
            .filter(UserCertification.id == access_attempt.certification_id)
            .first()
        )
        if certification:
            certification.successful_sessions += 1
            if access_attempt.session_duration_minutes:
                certification.total_usage_hours += (
                    access_attempt.session_duration_minutes / 60
                )
    ...
    return {
        ...
        "duration_minutes": access_attempt.session_duration_minutes,
    }
...
async def get_user_access_profile(
    ...
    recent_attempts = (
        db.query(MachineAccessAttempt)
        .filter(MachineAccessAttempt.user_id == user_id)
        .order_by(desc(MachineAccessAttempt.attempted_at))
        .limit(10)
        .all()
    )
...
@router.get(
    "/dashboard/equipment/{equipment_id}/stats",
    response_model=EquipmentAccessStats,
)
async def get_equipment_access_stats(
    ...
    attempts = (
        db.query(MachineAccessAttempt)
        .filter(MachineAccessAttempt.equipment_id == equipment_id)
        .all()
    )

    total_attempts = len(attempts)
    successful_attempts = len(
        [a for a in attempts if a.result == AccessAttemptResult.GRANTED]
    )
    ...
    access_rules = (
        db.query(MachineAccessRule)
        .filter(MachineAccessRule.equipment_id == equipment_id)
        .all()
    )
    ...
    incidents = (
        ...
        .filter(SafetyIncident.equipment_id == equipment_id)
        .count()
    )
...
async def get_makerspace_access_dashboard(
    ...
    total_certified = (
        db.query(UserCertification)
        .filter(UserCertification.status == CertificationStatus.ACTIVE)
        .count()
    )
    ...
    expired_certs = (
        db.query(UserCertification)
        .filter(UserCertification.status == CertificationStatus.EXPIRED)
        .count()
    )
    ...
    # Badge distribution
    badge_counts = (
        db.query(Badge.badge_type, func.count(UserBadge.id))
        .join(UserBadge)
    ...
    total_sessions = (
        db.query(MachineAccessAttempt)
        .filter(MachineAccessAttempt.result == AccessAttemptResult.GRANTED)
        .count()
    )

    incident_rate = (
        (total_incidents / total_sessions * 100) if total_sessions > 0 else 0
    )
