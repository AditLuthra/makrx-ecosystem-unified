...
async def create_permission(
    ...
    try:
        db_permission = crud_access_control.create_permission(
            db, permission, current_user.user_id
        )
...
async def update_permission(
    ...
    permission = crud_access_control.update_permission(
...
async def get_role(
    ...
    role = crud_access_control.get_role(
        db, role_id, _get_user_makerspace_id(current_user)
    )
    if not role:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_role(
    ...
    role = crud_access_control.update_role(
        db, role_id, role_update, current_user.user_id
    )
    if not role:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
@router.delete(
    "/roles/{role_id}/revoke/{user_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
async def get_user_access(
    ...
    if (
        not _has_permission(current_user, "view_members")
...
async def get_user_sessions(
    ...
    """Get user sessions"""
    if (
        not _has_permission(current_user, "view_sessions")
    ...
    sessions = crud_access_control.get_user_sessions(
...
async def terminate_session(
    ...
    if session.user_id != current_user.user_id and not _has_permission(
        current_user, "manage_sessions"
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def create_password_policy(
    ...
    try:
        db_policy = crud_access_control.create_password_policy(
            db, policy, current_user.user_id
        )
        return db_policy
    except Exception as e:
        raise HTTPException(
...
async def get_security_alerts(
    ...
    alerts = crud_access_control.get_security_alerts(
...
async def setup_default_roles(
    ...
    try:
        result = crud_access_control.setup_default_roles_and_permissions(
            db, makerspace_id
        )
        return {
...
async def send_role_assignment_notification(
    user_email: str, role_name: str, assigned_by: str
):
