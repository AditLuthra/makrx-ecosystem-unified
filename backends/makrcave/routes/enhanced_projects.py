...
async def get_public_projects(
    ...
    if required_equipment:
        for equipment in required_equipment:
            query = query.filter(
                models.Project.required_equipment.contains([equipment])
            )
    ...
    for project in projects:
        ...
        enhanced_project.completed_milestones_count = len(
...
@router.post(
    "/{project_id}/fork",
    response_model=ProjectForkResponse,
...
async def fork_project(
    ...
    forked_project = models.Project(
        ...
        required_skills=(
            original_project.required_skills.copy()
            if original_project.required_skills
            else []
        ),
        learning_objectives=(
            original_project.learning_objectives.copy()
            if original_project.learning_objectives
            else []
        ),
        license_type=original_project.license_type,
        required_equipment=(
            original_project.required_equipment.copy()
            if original_project.required_equipment
            else []
        ),
...
@router.post(
    "/{project_id}/bom/order",
    response_model=BOMOrderResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_bom_order(
    ...
    activity_log = models.ProjectActivityLog(
        ...
        metadata={
            "order_id": order.id,
            "quantity": order_data.quantity_ordered,
        },
    )
    db.add(activity_log)
...
@router.post(
    "/{project_id}/team-roles",
    response_model=ProjectTeamRoleResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.post(
    "/{project_id}/share-resource",
    response_model=ResourceSharingResponse,
    status_code=status.HTTP_201_CREATED,
)
async def share_resource(
    ...
    target_project = (
        db.query(models.Project)
        .filter(models.Project.project_id == sharing_data.target_project_id)
        .first()
    )
