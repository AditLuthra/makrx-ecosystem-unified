    ...
    try:
        ...
        if background_tasks and os.getenv("ENVIRONMENT") != "test":
            background_tasks.add_task(
                ...
                (
                    db_member.membership_plan.name
                    if db_member.membership_plan
                    else "Basic"
                ),
            )
...
async def update_member(
    ...
    member = crud_member.update_member(
        db, member_id, member_update, current_user.user_id
    )
    if not member:
        raise HTTPException(
...
async def create_member_invite(
    ...
    # Check if member already exists
    existing_member = crud_member.get_member_by_email(
        db, invite.email, invite.makerspace_id
    )
    if existing_member:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
async def send_member_invite_email(
    email: str, invite_token: str, plan_name: str, message: str = None
):
