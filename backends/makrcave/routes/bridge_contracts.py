...
async def record_filament_consumption(
    transaction: FilamentTransaction, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        if (
            transaction.delta_qty < 0
            and abs(transaction.delta_qty) > item.current_quantity
        ):
            raise HTTPException(
...
async def provider_accept_job(
    ...
    try:
        ...
        # Auto-notify Store
        await notify_store_status_change(
            job_id,
            JobStatusCallback(
                service_order_id=int(job.external_order_id),
                status="ACCEPTED",
