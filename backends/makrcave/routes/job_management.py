...
async def update_service_job(
    ...
    can_edit = (
        user_role in ["super_admin", "makerspace_admin"]
        or job.customer_id == user_id
        or job.assigned_provider_id == user_id
    )
...
async def upload_job_file(
    ...
    try:
        ...
        if gcode_metadata:
            ...
            db_file.estimated_print_time_gcode = gcode_metadata.get(
                "estimated_print_time_minutes"
            )
            db_file.estimated_material_usage_gcode = gcode_metadata.get(
...
async def create_status_update(
    ...
    try:
        ...
        elif status_update.new_status in [
            JobStatus.COMPLETED,
            JobStatus.CANCELLED,
            JobStatus.FAILED,
        ]:
        ...
        for line in lines:
            ...
            if "time" in line.lower() and any(
                ...
                if "minute" in line.lower() or "m" in line.lower():
                    ...
                    if time_match:
                        metadata["estimated_print_time_minutes"] = float(
                            time_match.group(1)
                        )

            # Look for material usage
            if "material" in line.lower() and any(
                unit in line.lower() for unit in ["g", "gram", "kg"]
            ):
                import re

                weight_match = re.search(r"(\d+(?:\.\d+)?)", line)
                if weight_match:
                    metadata["estimated_material_weight_grams"] = float(
