...
async def create_announcement(
    ...
    makerspace_id = (
        current_user.get("makerspace_id") if isinstance(current_user, dict)
    ...
    if announcement_data.summary:
        announcement_data.summary = InputSanitizer.sanitize_text(announcement_data.summary)
    announcement_data.content = InputSanitizer.sanitize_html(announcement_data.content)
    if announcement_data.action_button_text:
        announcement_data.action_button_text = InputSanitizer.sanitize_text(announcement_data.action_button_text)
    if announcement_data.action_button_url:
        announcement_data.action_button_url = InputSanitizer.sanitize_text(announcement_data.action_button_url)
    if announcement_data.event_location:
        announcement_data.event_location = InputSanitizer.sanitize_text(announcement_data.event_location)
    ...
    announcement = Announcement(
        ...
        created_by=(current_user.get('user_id') if isinstance(current_user, dict) else getattr(current_user, 'id', None)),
...
async def update_announcement(
    ...
    if "action_button_text" in update_data and update_data["action_button_text"]:
        update_data["action_button_text"] = InputSanitizer.sanitize_text(update_data["action_button_text"])
    if "action_button_url" in update_data and update_data["action_button_url"]:
        update_data["action_button_url"] = InputSanitizer.sanitize_text(update_data["action_button_url"])
    if "event_location" in update_data and update_data["event_location"]:
        update_data["event_location"] = InputSanitizer.sanitize_text(update_data["event_location"])

    # Update announcement fields
    for field, value in update_data.items():
        setattr(announcement, field, value)
    
    announcement.updated_by = current_user.get('user_id') if isinstance(current_user, dict) else getattr(current_user, 'id', None)
...
async def get_announcement_stats(
    ...
    # Check access permissions
    makerspace_id = (
        current_user.get('makerspace_id') if isinstance(current_user, dict) else getattr(current_user, 'makerspace_id', None)
    )
    if str(announcement.makerspace_id) != str(makerspace_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
