...
async def get_member_announcements(
    include_acknowledged: bool = Query(
        False, description="Include already acknowledged announcements"
    ),
    ...
    if not include_acknowledged:
        acknowledged_ids = (
            db.query(AnnouncementAcknowledgment.announcement_id)
            .filter(AnnouncementAcknowledgment.member_id == member.id)
            .all()
        )
    ...
    def sort_key(ann):
        ...
        return (
            ann.is_pinned,
            priority_order.get(ann.priority, 2),
            ann.created_at,
        )
    ...
    if member:
        ...
        if not existing_view:
            view = AnnouncementView(
                announcement_id=announcement.id, member_id=member.id
            )
            db.add(view)
...
async def create_announcement(
    ...
    makerspace_id = (
        current_user.get("makerspace_id")
        if isinstance(current_user, dict)
        else getattr(current_user, "makerspace_id", None)
    )
    if not makerspace_id:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if announcement_data.action_button_url:
        announcement_data.action_button_url = InputSanitizer.sanitize_text(
            announcement_data.action_button_url
        )
    if announcement_data.event_location:
        announcement_data.event_location = InputSanitizer.sanitize_text(
            announcement_data.event_location
        )
    ...
    if announcement_data.announcement_type == AnnouncementType.MAINTENANCE:
        if (
            not announcement_data.maintenance_start
            or not announcement_data.maintenance_end
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def update_announcement(
    ...
    if new_type == AnnouncementType.EVENT:
        event_date = (
            announcement_data.event_date
            if announcement_data.event_date is not None
            else announcement.event_date
        )
        if not event_date:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if new_type == AnnouncementType.MAINTENANCE:
        start = (
            announcement_data.maintenance_start
            if announcement_data.maintenance_start is not None
            else announcement.maintenance_start
        )
        end = (
            announcement_data.maintenance_end
            if announcement_data.maintenance_end is not None
            else announcement.maintenance_end
        )
    ...
    if "action_button_url" in update_data and update_data["action_button_url"]:
        update_data["action_button_url"] = InputSanitizer.sanitize_text(
            update_data["action_button_url"]
        )
    if "event_location" in update_data and update_data["event_location"]:
        update_data["event_location"] = InputSanitizer.sanitize_text(
            update_data["event_location"]
        )
...
async def get_announcement_stats(
    ...
    unique_viewers = (
        db.query(AnnouncementView.member_id)
        .filter(AnnouncementView.announcement_id == announcement_id)
        .distinct()
        .count()
    )
    ...
    total_acknowledgments = (
        db.query(AnnouncementAcknowledgment)
        .filter(AnnouncementAcknowledgment.announcement_id == announcement_id)
        .count()
    )
    ...
    if announcement.target_audience == TargetAudience.ALL_MEMBERS:
        target_members_count = (
            db.query(Member)
            .filter(Member.makerspace_id == announcement.makerspace_id)
            .count()
        )
