...
async def get_makerspace_settings(
    ...
    if not settings:
        ...
        settings = crud.create_default_settings(
            ...
            makerspace_name=(current_user.get('makerspace_name') if isinstance(current_user, dict) else getattr(current_user, 'makerspace_name', 'My Makerspace')),
            created_by=str(current_user.get('user_id') if isinstance(current_user, dict) else getattr(current_user, 'id', 'unknown'))
        )
...
async def update_makerspace_settings(
    ...
    settings = crud.update(
        ...
        updated_by=str(current_user.get('user_id') if isinstance(current_user, dict) else getattr(current_user, 'id', 'unknown'))
    )
