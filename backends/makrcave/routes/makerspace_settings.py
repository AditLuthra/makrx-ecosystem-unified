...
async def get_makerspace_settings(
    ...
    if not settings:
        ...
        settings = crud.create_default_settings(
            ...
            makerspace_name=(
                current_user.get("makerspace_name")
                if isinstance(current_user, dict)
                else getattr(current_user, "makerspace_name", "My Makerspace")
            ),
            created_by=str(
...
async def update_general_information(
    ...
    return await update_settings_section(
        "general", general_data.dict(exclude_unset=True), current_user, db
    )
...
async def update_access_control(
    ...
    return await update_settings_section(
        "access", access_data.dict(exclude_unset=True), current_user, db
    )
...
async def update_inventory_settings(
    ...
    return await update_settings_section(
        "inventory", inventory_data.dict(exclude_unset=True), current_user, db
    )
...
async def update_billing_config(
    ...
    return await update_settings_section(
        "billing", billing_data.dict(exclude_unset=True), current_user, db
    )
...
async def update_service_mode(
    ...
    return await update_settings_section(
        "service", service_data.dict(exclude_unset=True), current_user, db
    )
...
async def update_appearance(
    ...
    return await update_settings_section(
        "appearance",
        appearance_data.dict(exclude_unset=True),
        current_user,
        db,
    )
