...
async def register_provider(
    registration: ProviderRegistration, db: AsyncSession = Depends(get_db)
):
    """Register a new service provider"""
    try:
        # Validate makerspace exists
        makerspace_query = await db.execute(
            select(MakerspaceSettings).where(
                MakerspaceSettings.id == registration.makerspace_id
            )
        )
        ...
        provider = ServiceProvider(
            ...
            name=InputSanitizer.sanitize_text(registration.name),
            contact_email=registration.contact_email,
            status=ProviderStatus.ACTIVE,
            capabilities=registration.capabilities,
            max_build_volume=registration.max_build_volume,
            available_materials=[
                InputSanitizer.sanitize_text(m)
            ...
            location={
                k: InputSanitizer.sanitize_text(v)
                for k, v in registration.location.items()
            },
    ...
    try:
        ...
        for provider in providers_db.values():
            if (
                ...
                and material in provider.available_materials
                and quality in provider.quality_levels
            ):
        ...
        best_provider = min(
            available_providers, key=lambda p: (p.queue_length, -p.rating)
        )
        ...
        job = ServiceJob(
            ...
            notes=(
                InputSanitizer.sanitize_text(job_request.special_instructions)
                if job_request.special_instructions
                else None
            ),
            created_at=datetime.utcnow(),
            updated_at=datetime.utcnow(),
        ...
        return {
            ...
            "assigned_provider": {
                ...
                + timedelta(days=3 + best_provider.queue_length),
            },
            "job_details": {
...
async def notify_customer_job_update(
    customer_email: str, job_id: str, old_status: str, new_status: str
):
