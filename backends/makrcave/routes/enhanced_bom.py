...
async def list_bom_items(
    ...
    try:
        ...
        if filters.supplier_rating:
            query = query.filter(
                EnhancedBOMItem.supplier_rating.in_(filters.supplier_rating)
            )

        if filters.is_critical_path is not None:
            query = query.filter(
                EnhancedBOMItem.is_critical_path == filters.is_critical_path
            )

        if filters.is_long_lead_item is not None:
            query = query.filter(
                EnhancedBOMItem.is_long_lead_item == filters.is_long_lead_item
            )
        ...
        items = (
            query.order_by(desc(EnhancedBOMItem.added_at))
            .offset(offset)
            .limit(page_size)
            .all()
        )
...
async def get_bom_item(
    ...
    if not item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="BOM item not found"
        )
...
async def create_inventory_transaction(
    ...
    bom_item = (
        ...
        .filter(EnhancedBOMItem.id == transaction.bom_item_id)
        .first()
    )
    if not bom_item:
        raise HTTPException(
    ...
    try:
        ...
        bom_item.available_quantity = max(
            0, quantity_after - (bom_item.reserved_quantity or 0)
        )
...
async def record_usage(
    ...
    bom_item = (
        db.query(EnhancedBOMItem)
        .filter(EnhancedBOMItem.id == usage.bom_item_id)
        .first()
    )
    if not bom_item:
        raise HTTPException(
    ...
    try:
        ...
        bom_item.available_quantity = max(
            0, bom_item.current_stock_level - (bom_item.reserved_quantity or 0)
        )
...
async def get_bom_dashboard_stats(
    ...
    try:
        ...
        # Total value
        total_value = (
            db.query(func.sum(EnhancedBOMItem.total_cost))
            .filter(EnhancedBOMItem.total_cost.isnot(None))
            .scalar()
            or 0.0
        )
        ...
        for status in ProcurementStatus:
            count = base_query.filter(
                EnhancedBOMItem.procurement_status == status
            ).count()
        ...
        # Average lead time
        avg_lead_time = (
            db.query(func.avg(EnhancedBOMItem.supplier_lead_time_days))
            .filter(EnhancedBOMItem.supplier_lead_time_days.isnot(None))
            .scalar()
        )
    ...
    try:
        ...
        for item in items:
            if item.primary_supplier and item.total_cost:
                ...
                cost_by_supplier[supplier] = (
                    cost_by_supplier.get(supplier, 0) + item.total_cost
                )
        ...
        single_source_items = [
            item
            for item in items
            if item.primary_supplier and not item.alternative_suppliers
        ]
        if single_source_items:
            suggestions.append(
                f"Consider finding alternative suppliers for {len(single_source_items)} single-source items"
            )
        ...
        high_cost_no_alt = [
            ...
            if not items_with_cost[high_cost_items.index(item)].alternatives
        ]
        if high_cost_no_alt:
            suggestions.append(
                f"Review alternatives for {len(high_cost_no_alt)} high-cost items"
            )
