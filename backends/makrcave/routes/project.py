...
from models.project import Project, ProjectTask, ProjectMilestone, ProjectStatus, ProjectPriority
...
async def get_project_details(
    ...
    if not (project.is_public or 
            ...
            (project.assigned_members and user_id in project.assigned_members)):
        raise HTTPException(
...
async def create_project(
    ...
    try:
        ...
        new_project = Project(
            ...
            description=InputSanitizer.sanitize_html(project_data.get("description", "")) if project_data.get("description") else None,
            objectives=InputSanitizer.sanitize_text(project_data.get("objectives", "")) if project_data.get("objectives") else None,
            category=InputSanitizer.sanitize_text(project_data.get("category", "")) if project_data.get("category") else None,
            ...
            tags=[InputSanitizer.sanitize_text(t) for t in project_data.get("tags", [])]
        )
...
async def update_project(
    ...
    try:
        ...
        for field, value in project_data.items():
            if hasattr(project, field) and field not in ['id', 'project_id', 'creator_id', 'created_at']:
                if field in ['start_date', 'due_date', 'completion_date'] and value:
                    value = datetime.fromisoformat(value)
                elif field in ['status', 'priority']:
                    value = getattr(ProjectStatus if field == 'status' else ProjectPriority, value.upper())
