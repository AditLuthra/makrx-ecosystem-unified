        ...
        for bom_item in bom_items:
            try:
                ...
                if bom_item.part_code and bom_item.source == "store":
                    ...
                    matching_sku = await find_store_sku(
                        bom_item.part_code, bom_item.part_name
                    )
    ...
    try:
        async with get_store_client() as client:
            ...
            search_response = await client.get(
                "/catalog/products/search", params={"q": part_name, "limit": 1}
            )
    ...
    try:
        ...
        async with get_store_client() as client:
            ...
            if response.status_code in [200, 201]:
                logger.info(
                    f"Successfully added {len(cart_items)} items to cart for {user_email}"
                )
                return True
            else:
                logger.error(
                    f"Store cart API error: {response.status_code} - {response.text}"
                )
                return False
...
async def notify_export_completion(
    user_email: str, project_name: str, exported_count: int, skipped_count: int
):
