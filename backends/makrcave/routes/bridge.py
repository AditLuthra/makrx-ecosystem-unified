...
async def receive_job_from_store(
    job_request: StoreJobRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        logger.info(
            f"Created job {job.id} from Store order {job_request.external_order_id}"
        )
        ...
        if assigned_provider:
            ...
            logger.info(
                f"Auto-assigned job {job.id} to provider {assigned_provider['provider_id']}"
            )
        ...
        return {
            ...
            "assigned_provider": (
                assigned_provider["provider_id"] if assigned_provider else None
            ),
...
async def auto_assign_provider(
    ...
    try:
        ...
        for provider in mock_providers:
            if (
                material in provider["capabilities"]
...
async def notify_store_status_update(job: Job, status: str, db: Session):
    ...
    try:
        ...
        response = await notify_store_api(
            f"/bridge/jobs/{job.id}/status", "POST", notification_data
        )
...
async def sync_user_profile_from_store(
    profile_data: Dict[str, Any], db: Session = Depends(get_db)
):
