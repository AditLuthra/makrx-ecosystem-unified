...
async def get_analytics_overview(
    ...
    try:
        ...
        overview = {
            ...
            "peak_usage_hours": usage_data.get("peak_hours", [])[
                :5
            ],  # Top 5 peak hours
        }
...
async def get_trend_analysis(
    metric: str = Query(
        "revenue", description="Metric to analyze: revenue, usage, members"
    ),
    period: str = Query("daily", description="Period: daily, weekly, monthly"),
    ...
    try:
        ...
        if len(trend_data) >= 2:
            recent_avg = (
                sum(
                    item.get("revenue", item.get("unique_members", 0))
