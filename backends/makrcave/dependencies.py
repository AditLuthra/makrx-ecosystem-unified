...
KEYCLOAK_USE_JWKS = os.getenv("KEYCLOAK_USE_JWKS", "true").lower() in ("1", "true", "yes")
...
async def get_jwks_pem(kid: str) -> Optional[str]:
    ...
    now = time.time()
    if _jwks_cache and (now - _jwks_ts) < KEYCLOAK_PK_TTL_SECONDS and kid in _jwks_cache:
...
async def get_keycloak_public_key(force: bool = False):
    ...
    now = time.time()
    if (not force) and _keycloak_public_key and (now - _keycloak_public_key_ts) < KEYCLOAK_PK_TTL_SECONDS:
...
async def get_current_user(
    ...
    db_user = db.query(Member).filter(Member.keycloak_user_id == keycloak_user_id).first()
    if db_user is None:
        try:
            db_user = Member(
    ...
    roles = (
        ...
        + payload.get("resource_access", {}).get(KEYCLOAK_CLIENT_ID, {}).get("roles", [])
    )
    ctx = CurrentUser(
        ...
        first_name=payload.get("given_name"),
        last_name=payload.get("family_name"),
        makerspace_id=(db_user.makerspace_id if db_user else payload.get("makerspace_id")),
        roles=roles,
        model=db_user,
    )
    return ctx
...
class CurrentUser(dict):
    def __init__(self, user_id: str, email: str, first_name: Optional[str], last_name: Optional[str], makerspace_id: Optional[str], roles: List[str], model: Optional[Member]):
        super().__init__(
            user_id=user_id,
            email=email,
