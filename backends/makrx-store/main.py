...
try:
    ...
    if dsn:
        sentry_sdk.init(dsn=dsn, traces_sample_rate=float(os.getenv("SENTRY_TRACES_SAMPLE_RATE", "0.1")), environment=settings.ENVIRONMENT)
...
app.include_router(enhanced_catalog.router, prefix="/api/enhanced-catalog", tags=["Enhanced Catalog"])
...
async def startup_event():
    ...
    try:
        if os.getenv("ENVIRONMENT", "development") != "production":
            ...
            logger.info("Production mode: skipping auto table creation; use Alembic migrations", event="db_init")
...
async def init_sample_data():
    ...
    return {"message": "Sample data initialization would happen here", "success": True}
