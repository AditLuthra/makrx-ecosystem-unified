...
from models.commerce import Product, Category, OrderItem
from schemas import (
    ProductCreate,
    ProductUpdate,
    ProductSearch,
...
class ProductCRUD:
    ...
    async def update(
        self, db: AsyncSession, product_id: int, product_data: ProductUpdate
    ) -> Optional[Product]:
        """Update product"""
        try:
    ...
    async def search_products(
        self, db: AsyncSession, search: ProductSearch
                    ...
                    query = query.where(
                        or_(
                            and_(
                                ...
                                Product.sale_price >= filters.price_min,
                            ),
                            and_(
                                Product.sale_price.is_(None),
                                Product.price >= filters.price_min,
                            ),
                        )
                    )
                    count_query = count_query.where(
                ...
                if filters.price_max is not None:
                    query = query.where(
                        or_(
                            and_(
                                Product.sale_price.isnot(None),
                                Product.sale_price <= filters.price_max,
                            ),
                            and_(
                                Product.sale_price.is_(None),
                                Product.price <= filters.price_max,
                            ),
                        )
                    )
                    count_query = count_query.where(
