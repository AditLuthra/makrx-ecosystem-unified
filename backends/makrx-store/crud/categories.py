...
class CategoryCRUD:
    ...
    async def update(
        self, db: AsyncSession, category_id: int, category_data: CategoryUpdate
    ) -> Optional[Category]:
        """Update category"""
        try:
    ...
    async def reorder_categories(
        self, db: AsyncSession, category_orders: List[Dict[str, int]]
    ) -> bool:
        ...
        try:
            ...
            while current_category:
                ...
                if current_category.parent_id:
                    current_category = await self.get_by_id(
                        db, current_category.parent_id
                    )
                else:
                    break
