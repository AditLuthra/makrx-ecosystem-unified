    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not self.redis_client and settings.ENVIRONMENT == "production":
            ...
            if endpoint_type in critical:
                logger.error("Rate limiting backend unavailable in production; blocking critical endpoint request")
                return JSONResponse(
...
def setup_api_security(app: FastAPI):
    ...
    if redis_url:
        ...
        if settings.ENVIRONMENT == "production":
            logger.error("REDIS_URL is required in production for rate limiting and CSRF; critical endpoints will fail closed")
        else:
            logger.warning("REDIS_URL not set; rate limiting will fail open in development")
