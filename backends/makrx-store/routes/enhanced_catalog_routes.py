...
from models.commerce import Brand, Collection, CollectionProduct, Tag, ProductTag, Category, Product
...
async def get_brand_by_slug(
    ...
    try:
        brand_stmt = select(Brand).where(and_(Brand.slug == slug, Brand.is_active == True))
...
async def get_collection_by_slug(
    ...
    try:
        coll_stmt = select(Collection).where(and_(Collection.slug == slug, Collection.is_active == True))
        ...
        if collection.featured_categories:
            cats_stmt = select(Category).where(Category.id.in_(collection.featured_categories))
...
async def get_collection_products(
    ...
    try:
        ...
        # Apply sorting
        if sort == "featured":
            base_stmt = base_stmt.order_by(CollectionProduct.is_featured.desc(), CollectionProduct.sort_order)
        elif sort == "newest":
...
async def get_tag_info(
    ...
    try:
        ...
        product_ids_stmt = select(ProductTag.product_id).where(ProductTag.tag_id == tag.id)
        related_tags_stmt = (
            select(Tag.name)
