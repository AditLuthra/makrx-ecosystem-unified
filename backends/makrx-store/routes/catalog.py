...
def _serialize_product(product: Product) -> dict:
    ...
    return {
        ...
        # normalized key
        "stock_qty": getattr(product, "stock_qty", getattr(product, "stock_quantity", None)),
        "sku": product.sku,
        ...
        "effective_price": float(product.sale_price or product.price) if product.price else 0,
        "category": (
            {
...
async def get_products(
    ...
    base = select(Product).options(selectinload(Product.category)).where(Product.status == 'active')
    if category_id:
        base = base.where(Product.category_id == category_id)
    if search:
        like = f"%{search}%"
        base = base.where(or_(Product.name.ilike(like), Product.description.ilike(like)))

    # Count total
    count_stmt = select(func.count(Product.id)).select_from(Product).where(Product.status == 'active')
    if category_id:
        count_stmt = count_stmt.where(Product.category_id == category_id)
    if search:
        like = f"%{search}%"
        count_stmt = count_stmt.where(or_(Product.name.ilike(like), Product.description.ilike(like)))
...
async def get_categories(
    ...
    query = select(Category).where(Category.is_active == True).order_by(Category.sort_order, Category.name)
...
async def get_category_products(
    ...
    category_query = select(Category).where(Category.id == category_id, Category.is_active == True)
    category_result = await db.execute(category_query)
    category = category_result.scalar_one_or_none()
    
    if not category:
        return {"error": "Category not found", "products": [], "category": None}
