...
from core.unified_auth import get_current_user, get_idempotency_key, idempotency
from middleware.observability import audit, metrics, track_quote_to_order_conversion
from models.commerce import Order, Product
from models.services import ServiceOrder, Upload, Quote
...
async def complete_upload(
    ...
    try:
        ...
        res = await db.execute(
            select(Upload).where(
                and_(Upload.id == request.upload_id, Upload.user_id == current_user["keycloak_id"])
            )
        )
...
async def create_service_order_from_quote(
    ...
    try:
        ...
        qres = await db.execute(
            select(Quote).where(
                and_(Quote.id == request.quote_id, Quote.user_id == current_user["keycloak_id"])
            )
        )
...
async def publish_service_order_to_cave(
    ...
    try:
        ...
        sores = await db.execute(select(ServiceOrder).where(ServiceOrder.id == service_order_id))
        service_order = sores.scalars().first()
        ...
        quote = qres.scalars().first()
        ures = await db.execute(select(Upload).where(Upload.id == service_order.upload_id))
        upload = ures.scalars().first()
...
async def get_service_order_status(
    ...
    """
    try:
        sores = await db.execute(
            select(ServiceOrder).where(
                and_(ServiceOrder.id == service_order_id, ServiceOrder.user_id == current_user["keycloak_id"])
            )
        )
        service_order = sores.scalars().first()
