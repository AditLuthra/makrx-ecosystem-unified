...
from models.commerce import Order
from models.services import ServiceOrder
from schemas.services import ServiceOrderCreate, ServiceOrderUpdate
...
async def publish_job_to_makrcave(
    ...
    try:
        ...
        res = await db.execute(select(ServiceOrder).where(ServiceOrder.id == job_request.service_order_id))
...
async def update_job_status(
    ...
    try:
        ...
        res = await db.execute(
            select(ServiceOrder).where(cast(ServiceOrder.tracking["makrcave_job_id"], String) == job_id)
        )
        service_order = res.scalar_one_or_none()
