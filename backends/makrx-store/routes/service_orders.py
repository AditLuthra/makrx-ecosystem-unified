    ...
    try:
        ...
        for file in files:
            ...
            file_url = await upload_file_to_storage(
                file.file, file.filename, "3d-uploads"
            )

            # Schedule file analysis
            background_tasks.add_task(
                analyze_3d_file, file_url, file.filename, file.size
            )
...
async def create_service_order(
    ...
    try:
        ...
        # Create main order record matching simplified schema
        order = Order(
            order_number=f"MX-{datetime.utcnow().strftime('%Y%m%d')}-{str(uuid4())[:8].upper()}",
            user_id=quote_data["request"]["user_email"],
            ...
            or order_request.shipping_address,
            status="pending",
        ...
        # Create order item for the service (no product_id)
        order_item = OrderItem(
            order_id=order.id,
            product_id=None,
            quantity=quote_data["request"]["quantity"],
            price_at_time=quote_data["pricing"]["base_price"],
            total_price=quote_data["pricing"]["total"],
            meta={
        ...
        job_dispatch_result = await dispatch_job_to_providers(
            service_order_id, quote_data, order_request
        )
        ...
        if not job_dispatch_result["success"]:
            ...
            await db.commit()
            raise HTTPException(
                status_code=500, detail="Failed to dispatch job to providers"
            )
    ...
    try:
        async with get_makrcave_client() as client:
            ...
            response = await client.get(
                "/api/v1/providers/search", params=search_params
            )
    ...
    try:
        ...
        async with get_makrcave_client() as client:
            response = await client.post(
                "/api/v1/jobs/dispatch", json=job_request.dict()
            )
            ...
            if response.status_code in [200, 201]:
                ...
                logger.error(
                    f"Job dispatch failed: {response.status_code} - {response.text}"
                )
...
async def get_job_status_from_makrcave(
    service_order_id: str,
) -> Dict[str, Any]:
    """Get job status from MakrCave"""
    try:
        async with get_makrcave_client() as client:
            response = await client.get(
                f"/api/v1/jobs/by-service-order/{service_order_id}"
            )
...
async def process_service_order_payment(
    service_order_id: str, order_id: str, amount: float
):
