...
async def create_feature_flag(
    request: CreateFeatureFlagRequest, admin_user: str = Depends(require_admin)
):
    """
    ...
    try:
        ...
        if not validate_flag_value(request.default_value, request.flag_type):
            raise HTTPException(
                status_code=400, detail="Invalid default value for flag type"
            )
...
async def evaluate_single_feature_flag(
    ...
    try:
        ...
        return {"flag_key": flag_key, "value": value, "metadata": metadata}
...
async def get_feature_flag_analytics(admin_user: str = Depends(require_admin)):
    """
    ...
    try:
        ...
        for flag_data in feature_flags_store.values():
            flag_type = flag_data["flag_type"]
            analytics["flag_types"][flag_type] = (
                analytics["flag_types"].get(flag_type, 0) + 1
            )

        # Tag distribution
        for flag_data in feature_flags_store.values():
            for tag in flag_data.get("tags", []):
                analytics["tag_distribution"][tag] = (
                    analytics["tag_distribution"].get(tag, 0) + 1
                )
...
def evaluate_single_flag(
    flag_data: Dict[str, Any], context: FeatureFlagEvaluationRequest
    ...
    for i, rule in enumerate(flag_data.get("rules", [])):
        if evaluate_rule(rule, context):
            ...
            if rule["enabled"]:
                ...
                if (
                    flag_data["flag_type"] == "integer"
                    and rule["target_type"] == "user_role"
                ):
                    if rule["target_value"] == "premium":
                        return (
                            int(flag_data["default_value"]) * 2,
                            metadata,
