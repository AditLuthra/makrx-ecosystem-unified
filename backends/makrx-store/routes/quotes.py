...
from models.services import Quote, ServiceOrder
...
class MaterialProperties(BaseModel):
    ...
    supports_required: bool = Field(
        False, description="Whether supports are typically needed"
    )
    difficulty_multiplier: float = Field(
        1.0, description="Printing difficulty multiplier"
    )
...
class FileAnalysis(BaseModel):
    ...
    thin_walls_detected: bool = Field(
        False, description="Whether thin walls are detected"
    )
...
class QuoteCalculator:
    ...
    def calculate_material_cost(
        file_analysis: FileAnalysis, settings: PrintSettings
    ) -> Dict[str, float]:
        ...
        total_volume_mm3 = (
            solid_volume_mm3 + support_volume_mm3 + brim_volume_mm3
        ) * 1.1
    ...
    def calculate_print_time(
        ...
        quality_info = QUALITY_SETTINGS.get(
            settings.quality, QUALITY_SETTINGS["standard"]
        )
    ...
    def calculate_delivery_cost(
        delivery_address: Optional[Dict], pickup_location: Optional[str]
    ) -> Dict[str, float]:
...
async def create_quote(
    ...
    try:
        ...
        if quote_request.print_settings.rush_order:
            ...
            delivery_days = base_delivery_days + math.ceil(
                time_breakdown["total_time_hours"] / 24
            )
