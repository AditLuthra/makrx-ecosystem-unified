...
async def dispatch_job_to_providers(
    ...
    background_tasks.add_task(
        notify_providers, matching_providers, service_order, dispatch_id
    )
...
async def find_matching_providers(
    ...
    service_type = (
        ...
        if quote.service_type == "printing"
        else ServiceType.ENGRAVING
    )
...
def check_material_availability(
    ...
    for item in inventory_items:
        if (
            item.material_type == required_material
            and item.current_stock - item.reserved_stock >= quantity * 0.1
        ):  # Rough estimate
            return True
    return False
...
async def get_provider_capacity(db: AsyncSession, provider_id: str) -> int:
    ...
    provider_result = await db.execute(
        select(Provider.max_concurrent_jobs).where(Provider.id == provider_id)
    )
...
def calculate_match_score(
    provider: Provider, quote: Quote, distance: float, capacity: int
) -> float:
    """
...
async def send_job_notification(
    provider_match: ProviderMatch, service_order: ServiceOrder
):
    ...
    print(
        f"Sending notification to provider {provider_match.business_name}: {notification_data}"
    )
...
async def accept_job(
    ...
    if not dispatch_record:
        raise HTTPException(
            status_code=403, detail="You were not notified for this job"
        )
    ...
    await db.execute(
        ...
        .values(responded_at=datetime.utcnow(), response="accepted")
    )
...
async def reserve_materials(
    db: AsyncSession, provider_id: str, service_order: ServiceOrder
):
