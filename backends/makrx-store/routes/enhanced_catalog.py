    ...
    try:
        ...
        if request.filters:
            ...
            if filters.category_ids:
                base_stmt = base_stmt.where(Product.category_id.in_(filters.category_ids))
            ...
            if filters.rating_min:
                ...
                base_stmt = base_stmt.join(ProductRatingSummary, Product.id == ProductRatingSummary.product_id)
                base_stmt = base_stmt.where(ProductRatingSummary.average_rating >= filters.rating_min)
            
            if filters.has_reviews:
                review_subquery = (
                    select(Review.target_id)
                    .where(and_(Review.target_type == "product", Review.status == "published"))
                    .distinct()
                )
        ...
        if request.sort_by == "price_asc":
            base_stmt = base_stmt.order_by(func.coalesce(Product.sale_price, Product.price).asc())
        elif request.sort_by == "price_desc":
            base_stmt = base_stmt.order_by(func.coalesce(Product.sale_price, Product.price).desc())
        elif request.sort_by == "newest":
            base_stmt = base_stmt.order_by(Product.created_at.desc())
        elif request.sort_by == "rating":
            base_stmt = base_stmt.outerjoin(ProductRatingSummary, Product.id == ProductRatingSummary.product_id)
            base_stmt = base_stmt.order_by(ProductRatingSummary.average_rating.desc().nullslast())
        elif request.sort_by == "popularity":
            # Order by total orders (simplified)
            popularity_subquery = (
                select(OrderItem.product_id, func.sum(OrderItem.quantity).label('total_orders'))
                .group_by(OrderItem.product_id)
                .subquery()
            )
            base_stmt = base_stmt.outerjoin(popularity_subquery, Product.id == popularity_subquery.c.product_id)
            base_stmt = base_stmt.order_by(popularity_subquery.c.total_orders.desc().nullslast())
        else:  # relevance (default)
            base_stmt = base_stmt.order_by(Product.is_featured.desc(), Product.created_at.desc())
        ...
        for product in products:
            ...
            rating_stmt = select(ProductRatingSummary).where(ProductRatingSummary.product_id == product.id)
...
async def get_search_suggestions(db: AsyncSession, query: str, limit: int = 5) -> List[str]:
    ...
    prod_stmt = (
        select(Product.name).where(Product.name.ilike(f"%{query}%")).distinct().limit(limit)
    )
...
async def build_search_facets(db: AsyncSession, filters: Optional[AdvancedProductFilter]) -> List[SearchFacet]:
...
async def get_similar_products(db: AsyncSession, product_id: int, limit: int) -> List[Dict[str, Any]]:
    """Get products similar to the given product"""
    base_product = (await db.execute(select(Product).where(Product.id == product_id))).scalars().first()
    if not base_product:
        return []
...
async def get_complementary_products(db: AsyncSession, product_id: int, limit: int) -> List[Dict[str, Any]]:
...
async def get_trending_products(db: AsyncSession, category_id: Optional[int], limit: int) -> List[Dict[str, Any]]:
    ...
    trending = (await db.execute(stmt.order_by(Product.created_at.desc()).limit(limit))).scalars().all()
    
    return [{"id": p.id, "name": p.name, "price": float(p.price)} for p in trending]

async def get_personalized_recommendations(db: AsyncSession, user_id: str, limit: int) -> List[Dict[str, Any]]:
...
async def get_popular_products(db: AsyncSession, category_id: Optional[int], limit: int) -> List[Dict[str, Any]]:
    ...
    popular = (
        await db.execute(
            stmt.group_by(Product.id).order_by(func.count(OrderItem.id).desc()).limit(limit)
        )
    ).all()
...
async def find_compatible_products(db: AsyncSession, product: Product) -> List[Dict[str, Any]]:
