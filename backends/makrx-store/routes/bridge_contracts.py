...
async def publish_job_to_cave(
    request: ServiceOrderRequest, db: AsyncSession = Depends(get_db)
):
    """
...
def _verify_webhook(request: Request, raw_body: bytes) -> None:
    ...
    secret = getattr(settings, "CAVE_WEBHOOK_SECRET", None) or getattr(
    ...
    if not secret:
        ...
        return
    sig_header = request.headers.get("X-Signature") or request.headers.get(
        "X-MakrCave-Signature"
    )
    if not sig_header:
        if settings.ENVIRONMENT == "production":
            raise HTTPException(
