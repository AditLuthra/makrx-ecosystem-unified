    ...
    try:
        res = await db.execute(
            select(BOMIntegration).where(
                and_(BOMIntegration.id == import_id, BOMIntegration.makrcave_user_id == user_id)
            )
        )
...
async def apply_manual_mapping(
    ...
    try:
        ...
        for mapping in request.mappings:
            ...
            pres = await db.execute(select(Product).where(Product.id == mapping.store_product_id))
...
async def list_user_imports(
    ...
    """
    try:
        stmt = select(BOMIntegration).where(BOMIntegration.makrcave_user_id == user_id).order_by(BOMIntegration.created_at.desc())
...
async def map_bom_item_to_products(db: AsyncSession, bom_item: BOMItem) -> BOMMappingResult:
...
async def add_item_to_cart(
    ...
    ei = await db.execute(
        select(CartItem).where(and_(CartItem.cart_id == cart_id, CartItem.product_id == product_id))
    )
