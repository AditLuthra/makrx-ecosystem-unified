...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.Column('metadata', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.ForeignKeyConstraint(['order_id'], ['store_orders.id'], name='fk_order_item_order'),
        sa.ForeignKeyConstraint(['product_id'], ['store_products.id'], name='fk_order_item_product')
    )
