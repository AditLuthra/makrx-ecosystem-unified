...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['parent_id'], ['store_categories.id'], name='fk_category_parent', use_alter=True)
    )
    ...
    op.create_table(
        ...
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint('slug'),
        sa.ForeignKeyConstraint(['category_id'], ['store_categories.id'], name='fk_product_category')
    )
