...
def upgrade() -> None:
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_store_products_tags_gin ON store_products USING GIN (tags)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_store_products_attributes_gin ON store_products USING GIN (attributes)"
    )
