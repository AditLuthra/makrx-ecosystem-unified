...
class Provider(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: f"prv_{func.gen_random_uuid()}")
...
class ProviderInventory(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: f"inv_{func.gen_random_uuid()}")
    provider_id = Column(String, ForeignKey("providers.id"), nullable=False)
    
    # Material Info
    material_type = Column(String(100), nullable=False)  # "pla", "abs", "wood-mdf", etc.
...
class ServiceOrder(Base):
    """Service Orders sent to Providers"""
    __tablename__ = "service_orders"

    id = Column(String, primary_key=True, default=lambda: f"so_{func.gen_random_uuid()}")
    customer_id = Column(String, nullable=False)  # User who placed the order
    ...
    priority = Column(Integer, default=1)  # 1=normal, 2=rush
    
    # Delivery Info
    delivery_method = Column(String(50))  # "pickup", "delivery", "shipping"
    ...
    status_messages = Column(JSON, default=list)  # Timeline of status updates
    
    # Files and Documentation
    customer_files = Column(JSON)  # Original files from customer
    provider_images = Column(JSON, default=list)  # Progress photos
    gcode_generated = Column(Boolean, default=False)
    
    # Payments and Costs
    customer_price = Column(Float, nullable=False)
    provider_payment = Column(Float)  # What provider gets paid
    platform_fee = Column(Float)  # MakrX.Store commission
...
class JobDispatch(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: f"jd_{func.gen_random_uuid()}")
...
class ProviderReview(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: f"rv_{func.gen_random_uuid()}")
    service_order_id = Column(String, ForeignKey("service_orders.id"), nullable=False)
...
class MaterialCatalog(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: f"mat_{func.gen_random_uuid()}")
