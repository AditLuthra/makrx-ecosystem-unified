...
class SecretsManager:
    ...
    async def store_secret(
        ...
        try:
            ...
            if self.vault_url and self.vault_token:
                result = await self._store_secret_vault(
                    secret_name,
                    secret_value,
                    secret_type,
                    access_level,
                ...
                result = await self._store_secret_local(
                    secret_name,
                    secret_value,
                    secret_type,
                    access_level,
            ...
            await self._audit_secret_operation(
                "secret_created", secret_name, secret_type, access_level
            )
    ...
    async def retrieve_secret(
        self, secret_name: str, accessor_id: str, access_level: AccessLevel
    ) -> Optional[str]:
        """Retrieve secret with access control"""
        try:
            # Check access permissions
            if not await self._check_secret_access(
                secret_name, accessor_id, access_level
            ):
                await self._audit_secret_operation(
                    "access_denied",
                    secret_name,
            ...
            # Audit secret access
            await self._audit_secret_operation(
                "secret_accessed", secret_name, None, access_level, accessor_id
            )
    ...
    async def _check_secret_access(
        self, secret_name: str, accessor_id: str, required_level: AccessLevel
    ) -> bool:
...
class MFAManager:
    ...
    async def setup_mfa(self, user_id: str, user_email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            provisioning_uri = totp.provisioning_uri(
                name=user_email, issuer_name="MakrX Security"
            )
        ...
        try:
            ...
            if user_id in self.backup_codes:
                ...
                if token.upper() in backup_codes:
                    ...
                    await self._audit_mfa_operation(
                        "mfa_success", user_id, {"method": "backup_code"}
                    )
                    return True

            await self._audit_mfa_operation(
                "mfa_failure", user_id, {"token": token[:4] + "****"}
            )
            return False
    ...
    async def disable_mfa(self, user_id: str, admin_user_id: str) -> bool:
        ...
        try:
            ...
            await self._audit_mfa_operation(
                "mfa_disabled", user_id, {"admin_user_id": admin_user_id}
            )
            return True
    ...
    async def register_trusted_device(
        self, user_id: str, device_fingerprint: str, device_name: str = None
    ) -> str:
        ...
        await self._audit_mfa_operation(
            "device_registered", user_id, {"device_id": device_id}
        )
        return device_id
...
class AccessControlManager:
    ...
    async def create_admin_session(
        ...
        try:
            ...
            session_data = {
                ...
                "expires_at": (
                    datetime.utcnow()
                    + timedelta(seconds=OpSecConfig.ADMIN_SESSION_TIMEOUT)
                ).isoformat(),
            }
            ...
            await self._audit_access_operation(
                "admin_session_created", user_id, role, ip_address
            )
            ...
            await self._audit_access_operation(
                "admin_session_failed", user_id, role, ip_address, str(e)
            )
    ...
    async def _audit_access_operation(
        self,
        operation: str,
        user_id: str,
        role: str = None,
