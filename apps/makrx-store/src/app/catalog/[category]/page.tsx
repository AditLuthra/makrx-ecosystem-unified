...
import { products, categories, filterProducts, sortProducts, type Product, getProductsByCategory } from '@/data/products'
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.subcategories.map((subcat) => (
                    <Link key={subcat.slug} href={`/catalog/${categorySlug}?subcat=${subcat.slug}`}>
                    ...
                    <div key={filter.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                      ...
                      {filter.type === 'range' && (
                        <div className="space-y-3">
                          ...
                          {/* Range display removed: min/max/unit not present on CategoryFilter */}
                        </div>
                      )}
                      ...
                      {/* Toggle type not present in CategoryFilter type, UI removed */}
                    </div>
                  ))}
                </div>
              </div>
                          ...
                          <div className="relative">
                            ...
                            <div className="absolute bottom-2 right-2 flex gap-1">
                              <button 
                                ...
                                className={`p-2 rounded-full ${compareList.includes(product.id.toString()) ? 'bg-store-primary text-white' : 'bg-white/90 text-gray-600'} hover:bg-store-primary hover:text-white transition-colors`}
                              >
                          ...
                          <div className="flex-1 ml-4">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <p className="text-store-text-muted text-sm mb-2">{product.short_description}</p>
                                <div className="flex items-center gap-4 text-xs text-store-text-light">
                                  ...
                                  {product.inStock ? (
                                    <span className="text-green-600">In Stock ({product.stock_qty})</span>
                                  ) : (
                              ...
                              <div className="text-right">
                                ...
                                <div className="flex gap-2">
                                  <button 
                                    onClick={() => toggleCompare(product.id.toString())}
                                    className={`p-2 rounded ${compareList.includes(product.id.toString()) ? 'bg-store-primary text-white' : 'bg-gray-100 text-gray-600'}`}
                                  >
                                    <Scale className="h-4 w-4" />
                                  </button>
