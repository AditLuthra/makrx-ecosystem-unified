    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <input className="w-full border rounded px-3 py-2" value={userId} onChange={e=>setUserId(e.target.value)} placeholder="user-uuid" />
            </div>
            <div>
              <label className="block text-sm text-gray-700 mb-1">Email</label>
              <input className="w-full border rounded px-3 py-2" value={email} onChange={e=>setEmail(e.target.value)} placeholder="user@example.com" />
            </div>
            <div>
              <label className="block text-sm text-gray-700 mb-1">Type</label>
              <input className="w-full border rounded px-3 py-2" value={type} onChange={e=>setType(e.target.value)} placeholder="info, email, push" />
            </div>
            <div>
              <label className="block text-sm text-gray-700 mb-1">Status</label>
              <input className="w-full border rounded px-3 py-2" value={status} onChange={e=>setStatus(e.target.value)} placeholder="pending, sent, delivered" />
            </div>
            <div>
              <label className="block text-sm text-gray-700 mb-1">Read</label>
              <select className="w-full border rounded px-3 py-2" value={read} onChange={e=>setRead(e.target.value)}>
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <Button variant="ghost" onClick={()=>{setUserId('');setEmail('');setType('');setStatus('');setRead('');setDateFrom('');setDateTo(''); setPage(1);}}>Clear</Button>
          </div>
        </div>
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-6 text-center text-gray-500">No notifications found</td></tr>
              ) : (
                items.map(n => (
                  <tr key={n.id} className="hover:bg-gray-50">
                    ...
                    <Td>{n.type}</Td>
                    <Td>{n.status}</Td>
                    <Td className="max-w-xs truncate" title={n.subject || ''}>{n.subject || '-'}</Td>
                    <Td className="max-w-md truncate" title={n.message}>{n.message}</Td>
                    <Td>{n.read ? 'Yes' : 'No'}</Td>
                    <Td>{n.provider || '-'}</Td>
                  </tr>
                ))
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex items-center gap-2">
            <select className="border rounded px-2 py-1" value={perPage} onChange={e=>{setPerPage(parseInt(e.target.value)); setPage(1);}}>
