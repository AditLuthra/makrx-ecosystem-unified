...
export const useProjectPermissions = (project?: Project): ProjectPermissions => {
  ...
  return useMemo(() => {
    ...
    const isCollaborator = project
      ? (project.collaborators?.some((c) => c.user_id === userId) ?? false)
      : false;

    // Determine collaborator role
    const collaboratorRole = project
      ? project.collaborators?.find((c) => c.user_id === userId)?.role
      : null;

    // Determine if project is in user's assigned makerspace
    const isAssignedMakerspace = project ? currentMakerspace?.id === project.makerspace_id : true;
    ...
    const canViewAllProjects =
      hasPermission(userRole, 'projects', 'view', {
        isOwnResource: false,
    ...
    const projectPermissions: ProjectPermissions = {
      ...
      canDelete:
        canDeleteProjects && (isOwner || ['super_admin', 'makerspace_admin'].includes(userRole)),
      canAddMembers:
        canAddMembers &&
        hasProjectAccess &&
        (isOwner ||
      ...
      canReserveEquipment:
        canReserveEquipment &&
        hasProjectAccess &&
        (isOwner || isCollaborator || ['super_admin', 'makerspace_admin'].includes(userRole)),
      canManageFiles:
        canManageFiles &&
        hasProjectAccess &&
        (isOwner ||
...
export const canAccessProject = (user: any, project: Project, currentMakerspace: any): boolean => {
