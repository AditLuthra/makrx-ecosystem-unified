...
const EquipmentCheckInOut: React.FC<EquipmentCheckInOutProps> = ({ className = '' }) => {
  ...
  const handleCheckIn = async (equipmentId: string) => {
    ...
    const existingSession = sessions.find(
      (s) => s.equipment_id === equipmentId && s.user_id === user?.id && s.status === 'active',
    );
  ...
  const updateConsumable = (
    index: number,
    field: keyof (typeof consumables)[0],
    value: string | number,
  ) => {
    setConsumables((prev) =>
              ...
              .map((session) => (
                ...
                >
                  ...
                  <div className="text-right text-sm text-gray-600">
                    ...
                    <p>
                      {format(new Date(session.check_in_time), 'MMM d, HH:mm')} -{' '}
                      {session.check_out_time && format(new Date(session.check_out_time), 'HH:mm')}
                    </p>
                  </div>
                </div>
              ))}
