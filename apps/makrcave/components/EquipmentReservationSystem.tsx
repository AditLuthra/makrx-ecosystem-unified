...
const EquipmentReservationSystem: React.FC<EquipmentReservationSystemProps> = ({
  className = '',
}) => {
  const { user } = useAuth();
  ...
  const loadReservationsData = async () => {
    ...
    try {
      ...
      const mockAccessPolicies: EquipmentAccessPolicy[] = equipment.map((eq) => ({
        ...
        max_daily_cap:
          eq.type === 'laser_cutter' ? 500 : eq.type === 'cnc_machine' ? 800 : undefined,
  ...
  const handleCreateReservation = async () => {
    ...
    const newReservation: EquipmentReservation = {
      ...
      equipment_name:
        equipment.find((e) => e.id === formData.equipment_id)?.name || 'Unknown Equipment',
      user_id: user?.id || 'current-user',
      user_name: formatUserDisplayName(user),
      start_time: startDateTime.toISOString(),
      end_time: endDateTime.toISOString(),
      status: policies.find((p) => p.equipment_id === formData.equipment_id)?.auto_approve
        ? 'approved'
        : 'pending',
              ...
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                <div className="space-y-2">
                  {filteredReservations
                    .filter(
                      (res) =>
                        format(new Date(res.start_time), 'yyyy-MM-dd') ===
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {accessPolicies.find(
                              (p) => p.equipment_id === reservation.equipment_id,
                            ) && (
