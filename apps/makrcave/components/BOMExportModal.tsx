...
export default function BOMExportModal({
  ...
  const loadExportPreview = async () => {
    ...
    try {
      const response = await api.get(`/api/v1/projects/${projectId}/bom/export/preview`);
  ...
  const handleSelectAll = (exportableOnly = false) => {
    if (exportableOnly) {
      const exportableIds = exportPreview.filter((item) => item.exportable).map((item) => item.id);
      setSelectedItems(new Set(exportableIds));
    } else {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.post(`/api/v1/projects/${projectId}/bom/export`, exportRequest);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <Tabs
          value={activeTab}
          onValueChange={(value) => setActiveTab(value as 'preview' | 'results')}
        >
            ...
            {isLoadingPreview ? (
              ...
              <>
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>{exportableCount}</strong> of <strong>{exportPreview.length}</strong>{' '}
                    BOM items can be exported to the Store. Items must have valid SKUs or part codes
                    that match Store products.
                  </AlertDescription>
                </Alert>
                ...
                >
                  ...
                  <AlertDescription>
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        📦 Total Items: {exportResult.exported_items + exportResult.skipped_items}
                      </div>
                    </div>
                  </AlertDescription>
                </Alert>
