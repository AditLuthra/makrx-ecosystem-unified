...
const SmartInventory: React.FC = () => {
  ...
  const loadInventoryData = async () => {
    try {
      ...
      const [invRes, fcRes, poRes, supRes] = await Promise.all([
        fetch('/api/v1/inventory', { headers }),
        fetch('/api/v1/inventory/forecasts', { headers }),
        fetch('/api/v1/purchase-orders', { headers }),
        fetch('/api/v1/suppliers', { headers }),
      ]);

      const [inv, fc, po, sup] = await Promise.all([
        invRes.ok ? invRes.json() : Promise.resolve([]),
        fcRes.ok ? fcRes.json() : Promise.resolve([]),
        poRes.ok ? poRes.json() : Promise.resolve([]),
        supRes.ok ? supRes.json() : Promise.resolve([]),
      ]);
