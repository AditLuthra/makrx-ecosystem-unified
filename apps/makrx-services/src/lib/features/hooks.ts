...
export function useFeatureCheck(
  ...
  const [availability, setAvailability] =
    useState<FeatureAvailabilityResponse | null>(null);
...
export function useUserFeatures(
  ...
  const loadFeatures = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const result = await featureFlagsClient.getAvailableFeatures(
        context,
        tags,
      );
      setFeatures(result);
 ...
 */
export function useServiceFeatureManager(
  ...
  const enabledFeatures = useMemo(() => {
    if (!features) return [];
    return relevantFeatures.filter((feature) =>
      features.enabled.includes(feature),
    );
  ...
  const betaFeatures = useMemo(() => {
    if (!features) return [];
    return relevantFeatures.filter((feature) =>
      features.beta.includes(feature),
    );
  }, [features, relevantFeatures]);
...
export function useAdminFeatureFlags(authToken?: string) {
  ...
  const updateFlag = useCallback(
    async (
      ...
      try {
        await featureFlagsClient.updateFeatureFlag(
          featureKey,
          updates,
          authToken,
        );
  ...
  const createOverride = useCallback(
    async (featureKey: string, accessLevel: AccessLevel) => {
      if (!authToken) return;

      try {
        await featureFlagsClient.createRuntimeOverride(
          featureKey,
          accessLevel,
          authToken,
        );
  ...
  const bulkUpdate = useCallback(
    async (flagsWithLevels: Record<string, AccessLevel>) => {
      if (!authToken) return;

      try {
        await featureFlagsClient.bulkUpdateFeatureFlags(
          flagsWithLevels,
          authToken,
        );
        await loadAllFlags(); // Refresh after update
...
export function useFeatureFlagDebug() {
  ...
  const updateTestContext = useCallback(
    (updates: Partial<FeatureAccessContext>) => {
