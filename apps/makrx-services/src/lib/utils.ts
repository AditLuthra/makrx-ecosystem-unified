...
export function validateSTLFile(file: File): {
  isValid: boolean;
  error?: string;
} {
  // Check file type
...
export function validateSVGFile(file: File): {
  isValid: boolean;
  error?: string;
} {
  // Check file type
...
export function getServiceTypeFromFile(
  file: File,
): "printing" | "engraving" | null {
...
export function calculateEstimatedPrice(
  serviceType: "printing" | "engraving",
  material: string,
  quantity: number,
  volume?: number,
  area?: number,
  ...
  if (serviceType === "printing" && volume) {
    ...
    const pricePerUnit =
      basePrices.printing[material as keyof typeof basePrices.printing] || 150;
  ...
  if (serviceType === "engraving" && area) {
    const pricePerCm2 =
      basePrices.engraving[material as keyof typeof basePrices.engraving] || 50;
