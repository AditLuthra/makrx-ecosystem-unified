...
export async function GET(request: NextRequest, { params }: { params: { slug: string } }) {
  ...
  if (isMockMode()) {
    ...
    if (track) filteredEvents = filteredEvents.filter((e) => e.track === track);
  ...
  try {
    // Replace with actual DB call wrapped in safeDbCall
    const dbEvents = await safeDbCall(async () => {
      // TODO: Replace with real DB query
      // Example: return await db.query(...)
      return mockSubEvents; // fallback to mock for now
    }, mockSubEvents);
