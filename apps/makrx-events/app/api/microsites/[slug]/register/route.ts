...
export async function POST(
  ...
  try {
    ...
    const [subEvent] = await db
      ...
      .from(subEvents)
      .where(and(
        eq(subEvents.id, validatedData.subEventId),
        eq(subEvents.micrositeId, microsite.id)
      ))
      .limit(1);
    ...
    if (validatedData.participantInfo.email) {
      try {
        await sendRegistrationConfirmationEmail(
          validatedData.participantInfo.email,
          {
    ...
    const response = {
      ...
      nextSteps: subEvent.registrationType === 'paid' 
        ? {
