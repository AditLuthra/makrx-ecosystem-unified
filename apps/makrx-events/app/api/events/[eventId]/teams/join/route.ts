...
export async function POST(request: NextRequest, { params }: { params: { eventId: string } }) {
  try {
    ...
    const [team] = await db
      .select()
      .from(teams)
      .where(and(eq(teams.eventId, params.eventId), eq(teams.inviteCode, validatedData.inviteCode)))
      .limit(1);
    ...
    const existingMember = await db
      .select()
      .from(teamMembers)
      .where(and(eq(teamMembers.teamId, team.id), eq(teamMembers.userId, validatedData.userId)))
      .limit(1);
