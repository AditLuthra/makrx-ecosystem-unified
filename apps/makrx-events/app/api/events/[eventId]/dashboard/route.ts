...
export async function GET(
  ...
  try {
    ...
    const registrationTrend = await db
      ...
      .from(eventRegistrations)
      .where(
        gte(eventRegistrations.registeredAt, oneDayAgo)
      )
...
function getActivityDetails(activity: string): string {
  switch (activity) {
    case 'register': return 'registered for the event';
    case 'check_in': return 'checked in to the event';
    case 'cancel': return 'cancelled their registration';
    case 'view': return 'viewed the event page';
