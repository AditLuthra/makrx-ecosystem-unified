...
export default async function MicrositeVenuePage({ params }: MicrositeVenuePageProps) {
  ...
  const venue = {
    ...
    description: "The Moscone Center is San Francisco's premier convention and exhibition facility, located in the heart of the South of Market district. With state-of-the-art facilities and flexible spaces, it's the perfect venue for MakerFest 2024.",
    
    facilities: [
      { icon: Wifi, name: "High-Speed WiFi", description: "Complimentary WiFi throughout the venue" },
      { icon: Utensils, name: "Food & Beverage", description: "Multiple dining options and food trucks on-site" },
      { icon: Accessibility, name: "Accessibility", description: "Full ADA compliance and accessibility features" },
      { icon: Car, name: "Parking", description: "On-site parking garage with electric vehicle charging" }
    ],
    ...
    transportation: {
      ...
      driving: {
        name: "Driving",
        directions: "From 101: Take I-80 W toward Bay Bridge, exit at 4th St. From 280: Take US-101 N, exit at 9th St",
        icon: Car
      },
      airport: {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(venue.floorPlan).map(([space, description]) => (
                <div key={space} className="p-4 bg-gray-50 rounded-lg">
                  <h3 className="font-semibold mb-2 capitalize">{space.replace(/([A-Z])/g, ' $1')}</h3>
