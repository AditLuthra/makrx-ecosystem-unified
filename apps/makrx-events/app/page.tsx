  ...
  const heroSlides = [
    {
      ...
      description: 'Join the global maker movement. Find workshops, competitions, and exhibitions near you or create your own technical fest.',
    ...
    },
    {
      ...
      description: 'Join us for the biggest maker event of the year with workshops, competitions, and networking!',
    ...
    },
    {
      ...
      description: 'Show off your robotics skills in this international competition with amazing prizes.',
            ...
            <div className="flex flex-col items-center justify-center space-y-6">
              ...
              <div className="h-10 flex items-center justify-center">
                {heroSlides[currentSlide]?.type !== 'main' && (
                  <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold transition-all duration-800 ease-in-out ${
                    heroSlides[currentSlide]?.type === 'event' ? 'bg-blue-500/20 border border-blue-400 text-blue-200' :
                    heroSlides[currentSlide]?.type === 'competition' ? 'bg-red-500/20 border border-red-400 text-red-200' :
                    'bg-green-500/20 border border-green-400 text-green-200'
                  }`}>
                    {heroSlides[currentSlide]?.type === 'event' ? 'Featured Event' :
                     heroSlides[currentSlide]?.type === 'competition' ? 'Competition' : 'Workshop'}
                  </span>
                )}
              </div>
              ...
              <div className="min-h-16 sm:min-h-20 lg:min-h-24 flex items-center justify-center">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-center transition-all duration-800 ease-in-out leading-tight">
                  <span className="block">{heroSlides[currentSlide]?.title || 'Discover & Create'}</span>
              ...
              <div className="min-h-8 sm:min-h-12 flex items-center justify-center">
                <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-200 max-w-2xl mx-auto text-center transition-all duration-800 ease-in-out px-4">
                  {heroSlides[currentSlide]?.description || 'Join the global maker movement. Find workshops, competitions, and exhibitions near you or create your own technical fest.'}
                </p>
              </div>
              
              {/* Event details area - responsive height */}
              <div className="min-h-8 sm:min-h-10 flex items-center justify-center">
                {heroSlides[currentSlide]?.type !== 'main' && heroSlides[currentSlide]?.location && heroSlides[currentSlide]?.date && (
                  <div className="flex flex-col sm:flex-row gap-3 justify-center items-center text-gray-300 text-sm transition-all duration-800 ease-in-out">
                    ...
                    <div className="flex items-center">
                      ...
                      {heroSlides[currentSlide]?.date && new Date(heroSlides[currentSlide].date).toLocaleDateString()}
                    </div>
                  </div>
                )}
              </div>
              ...
              {heroSlides[currentSlide]?.type === 'main' ? (
                ...
                <>
                  <Button 
                    ...
                    onClick={() => {
                      const slug = heroSlides[currentSlide]?.id === '1' ? 'maker-fest-2024' :
                                   heroSlides[currentSlide]?.id === '2' ? 'robotics-championship' :
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            <div className="text-center group cursor-pointer">
              <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform group-hover:scale-105 border border-gray-100">
                ...
                <div className="text-xs sm:text-sm lg:text-base text-gray-600 font-medium">Active Events</div>
              </div>
            </div>
            <div className="text-center group cursor-pointer">
              <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform group-hover:scale-105 border border-gray-100">
                ...
                <div className="text-xs sm:text-sm lg:text-base text-gray-600 font-medium">Global Cities</div>
              </div>
            </div>
            <div className="text-center group cursor-pointer">
              <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform group-hover:scale-105 border border-gray-100">
                ...
                <div className="text-xs sm:text-sm lg:text-base text-gray-600 font-medium">Registered Makers</div>
              </div>
            </div>
            <div className="text-center group cursor-pointer">
              <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform group-hover:scale-105 border border-gray-100">
                ...
                <div className="text-xs sm:text-sm lg:text-base text-gray-600 font-medium">Total Workshops</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 sm:mb-16">
            ...
            <p className="text-lg sm:text-xl text-gray-600 max-w-3xl mx-auto">
              Join thousands of makers, creators, and innovators at events happening around the world
            </p>
          </div>
            ...
            {featuredEvents.map((event, index) => (
              ...
              >
                ...
                <CardContent className="p-4 sm:p-6 lg:p-8">
                  ...
                  <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-5 leading-relaxed">{event.shortDescription}</p>
                  ...
                  <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0">
                    <Button size="sm" className="w-full bg-gradient-to-r from-primary to-blue-600 hover:from-primary/90 hover:to-blue-600/90 text-white font-medium py-2.5 sm:py-3 text-xs sm:text-sm rounded-lg sm:rounded-xl shadow-lg hover:shadow-xl transition-all duration-300">
